...
export const handleDeliver: PhaseHandler = async (input): Promise<PhaseOutcome> => {
  ...
  const pr = existing === null ? await openPullRequest(input, branch, body) : await refresh(input, existing, body)
...
/** Keeps a reused pull request's body in step with the latest report. */
const refresh = async (input: PhaseInput, pr: PullRequestRef, body: string): Promise<PullRequestRef> => {
  await input.deps.github.updatePullRequest(pr.number, { body })
  return pr
}

const renderPrBody = (input: PhaseInput): string => {

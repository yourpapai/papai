...
| `ANTHROPIC_API_KEY`                        | claude route: exactly one of this and the token | —                                                                               | Secret. Anthropic Console API key — the documented default spelling (Commercial Terms, predictable per-token billing)                                                                                                                                                                     |
| `CLAUDE_CODE_OAUTH_TOKEN`                  | claude route: exactly one of this and the key   | —                                                                               | Secret. Subscription OAuth token for Pro/Max/Team/Enterprise billing; see [Backend selection](#backend-selection-the-claude-cli-route) for the `--bare` caveat                                                                                                                            |
...
the default route is byte-identical to the pre-knob pipeline. Set exactly one
Anthropic secret (`ANTHROPIC_API_KEY` or `CLAUDE_CODE_OAUTH_TOKEN`): both set
fails at startup because the API key silently wins billing, and neither set
fails because no credential remains. A set `LLM_API_KEY` is refused outright on
this route — with no provider proxy in front of the review loop's
`opencode run` children, a present gateway key would reach a subprocess whose
children the model controls.
...
- **The chosen credential is readable by the CLI's own `Bash` children.** The
  ...
  exclusivity: prefer the revocable, spend-capped Console API key on
  threat-model-sensitive repositories; the OAuth token carries a subscription's
  spend and a ~12-month lifetime, and rotating it means regenerating it (there
  is no automation, by design).
...
- **One caveat pending the credentialed recording.** The pinned CLI's own
  `--bare` help states that under `--bare`, "Anthropic auth is strictly
  `ANTHROPIC_API_KEY` or apiKeyHelper … (OAuth and keychain are never read)":
  an un-credentialed probe on 2.1.239 confirms the env token is not consulted
  (`apiKeySource: "none"` with `CLAUDE_CODE_OAUTH_TOKEN` set). Whether the
  OAuth spelling can serve this route at all is settled by the recorder run
  (`bun run opencode-agent:test:claude-live`), which the fixture corpus's
  `VERSION` stamp and the workflow pin then reflect together.

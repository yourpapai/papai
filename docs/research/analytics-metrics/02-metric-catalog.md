...
| `tool_started`               | `tool_slug`; `tool_key`; `origin: core\|first_party_plugin\|external_plugin\|user_mcp`; `domain: task\|memo\|schedule\|attachment\|web\|identity\|coding\|config\|meta\|diagnostics\|other`; `risk: read\|write\|destructive\|open_world`; `model_role: main\|small`; `args_bytes: ByteBucket`                                      |

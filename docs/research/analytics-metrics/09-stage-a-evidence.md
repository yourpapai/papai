...
| 16 | DSAR/delete/rekey/snapshot | partial | DSAR export + deletion workflow + encrypted target bundles (d994c6f7e); rekey workflow incl. shadow equation + retirement gating (193327d5b + 1cdb28106); curated snapshot publisher + fail-closed SnapshotConsumerCoordinator + five reviewed Metabase models (3361cd9fb + fix 026f5be3f); REMAINING: production fence admission wiring (Task 17) + delivery worker admission (Task 15) |
...
| 15 — aggregate delivery | 48 pass / 0 fail (http-policy, aggregate-release, delivery-worker, captured-egress named gates); mirrored delivery suites (release-suppression, pinned-transport, worker-send, delivery-lifecycle, aggregate-delivery store/classify) + full tests/analytics 1206 pass | clean / clean (knip clean, format clean, security 0 findings) | 4a1ecab2a | 2026-07-27 |

...
| 16 | DSAR/delete/rekey/snapshot | green | DSAR export + deletion workflow + encrypted target bundles (d994c6f7e); rekey workflow: plan/apply/verify/abort CLI, dual-write fenced seam, encrypted domain-complete mappings, FK-ordered copy, cutover fence, shadow equation, swap tx, retirement gating (193327d5b + 1cdb28106); curated snapshot publisher (fresh-empty allowlisted schema, byte/schema/freelist canary scan, mode-0600 staging, atomic rename, rekey-owned staged token) + fail-closed SnapshotConsumerCoordinator (reopen-proof, orphan reconciliation, rekey cutover quiesce, open-inode unlink refusal) + five reviewed Metabase SQL models with full honesty block (3361cd9fb); PRODUCTION fence admission wiring lands with Task 17 job registration + Task 15 delivery worker; 2026-07-27 |
...
| 14 — snapshot/metabase | 193 pass / 0 fail (snapshot, metabase-models, friction-sample, rekey, rekey-cutover aggregators); full tests/analytics 1110 pass | clean / clean (security 0 findings) | 3361cd9fb | 2026-07-27 |

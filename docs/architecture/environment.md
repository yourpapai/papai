...
**Optional runtime flags:** `DEBUG_SERVER`, `DEBUG_HOSTNAME`, `DEBUG_PORT`, `LOG_LEVEL`. `KANEO_CLIENT_URL`/`KANEO_INTERNAL_URL` are bootstrap-only: read solely by the legacy-repair path (`src/instances/kaneo-legacy-repair.ts`) when it creates the default Kaneo task instance on first start. The Kaneo provisioning route (`src/debug/settings/provision-routes.ts`) sources `publicUrl`/`internalUrl` from the bound task instance's own `config` (`baseUrl`/`internalUrl`) instead of these env vars, so multi-instance deployments provision against the correct instance; `KANEO_INTERNAL_URL` also carries internal bot-to-Kaneo traffic.

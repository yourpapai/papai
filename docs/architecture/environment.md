...
- `LLM_BASE_PROVIDER`, `LLM_BASE_MODEL` — optional declared base references for the env-seeded provider (e.g. `openai` + `gpt-4o` when `MAIN_MODEL` is a deployment-specific alias); applied **only when the env seed creates the provider** (trimmed; absent/empty → null) and never patched onto an existing one — edit the provider in the settings admin "Providers" section instead. They feed the models.dev catalogue lookup (below), not the API request itself.

**models.dev catalogue (no env vars):** on boot the bot prewarms an in-memory snapshot of `https://models.dev/api.json` (10 s fetch timeout, 60-min TTL, background refresh) with a disk cache at `~/.cache/papai/models.json`; a failed refresh keeps the previous snapshot and an empty snapshot disables catalogue lookups (resolved limits fall back to the built-in prefix table). The host and cache path are fixed constants (`src/models-dev/client.ts`), not configurable.

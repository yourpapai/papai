...
- `bun build:client` — bundle `client/{debug,admin,settings}/` to `public/` (`CLIENT_BUILD_OUTDIR` overrides the output dir; used by tests to build into a temp dir). The debug-server test suites **fail fast** if `public/` bundles are missing instead of building them. `bun check:full` and `bun test:coverage` run `scripts/ensure-client-built.ts` first and build the bundles automatically when they're missing; `bun run test` does not self-build, so run `bun build:client` once before it on a clean checkout.

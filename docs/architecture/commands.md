...
The story launcher starts Bun with `--no-env-file` and runs it through a required operating-system sandbox. macOS uses a `sandbox-exec` profile; Linux uses the pinned `docker.io/oven/bun:1.3.13@sha256:87416c977a612a204eb54ab9f3927023c2a3c971f4f345a01da08ea6262ae30e` OCI image with read-only source/dependency mounts, no network, dropped capabilities, and no-new-privileges. Unsupported platforms fail closed rather than falling back to JavaScript-only isolation. The child inherits only sanitized runner variables, works from the captured `app/` snapshot, and can write only its session `tmp/` root and exact pre-created report files. The dependency cache is keyed by raw `package.json`, raw `bun.lock`, and Bun version; the report manifest records its key and tree fingerprint alongside frozen-harness and candidate-runtime hashes. The preload remains defense-in-depth diagnostics for scenarios, resource leaks, and declared HTTP routes; it is not the hermeticity boundary.

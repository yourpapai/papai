...
The `afk-runner/` workspace is the graph-kernel re-host of the SDD pipeline: lifecycle states, transitions, and guards declared as **data** over XState v5 used purely as a reducer, with machine state derived exclusively by folding the append-only `events.ndjson`. Since C3 the graph **drives live think-half runs** (intake → draft → review) through a generic, stage-agnostic loop — resume is the same loop re-folding the log. Since C4 a presented gate is **machine state** (`gate.awaiting`): four settle producers answer through one validated seam, and the run continues per outcome — approve, extend, veto, or abort. It exists beside `sdd-runner/` (which stays frozen until a retirement change), not inside it; work modules are copies, never imports. Planning artifacts for the whole C1–C7 effort live in `openspec/changes/afk-runner/` (proposal, design, tasks, `specs/afk-runner-kernel/spec.md`); C3's decisions live in `openspec/changes/think-half-on-graph/`; C4's in `openspec/changes/gate-as-state/`; this page describes the implemented state and the decisions future changes inherit.
...
- `afk-runner/src/graph/` — pipeline v0 as per-state data modules (`states/pipeline-states.ts`) composed in `pipeline.ts`; `pipeline-work.ts` composes the per-state **work modules** (work kind + outcome→successor data, context-pure outcome readers) into the `workFor` registry the loop consumes. Since C4 `gate` is compound — `awaiting` is the positional park of a presented gate; review carries the interstitial `gate.presented` edge (no stage-map writes) and awaiting's exits are the legacy mover events.
- `afk-runner/src/drive/` — `loop.ts` (the generic successor-or-park drive loop; names no stage; compound positions flatten to dot-paths — `gate.awaiting`; self-successors re-run a state's own work, the extended-round path), `boundary.ts` (append boundary: pure transition probe, refusal throws), `resume.ts` (resume derivation as pure functions of folded context + session ledger; owed-mover recovery for the crash window between an explicit-outcome answer and its mover).
- `afk-runner/src/work/` — think-half work re-hosted as edge work (copies from sdd-runner): intake, draft (+ `stage-halt.ts`; C4: a settled veto re-enters draft as veto-updater revision work), review (`review.ts` wrapper around the review-loop copy: re-entry from context+ledger, cap-hit presents the full early gate — MD + hashes sidecar + `presented` — then runs the ladder prelude), review-loop, review-model, steer, materialize (+ `digest-format.ts`); the C4 gate modules: `gate-model`/`gate-answers`/`gate-render`/`gate-digest-extract`/`gate-files` (rendering + hashes sidecar), `gate-signals` (findings + cost summary), `gate-settle` (the one settle seam), `gate-claims` (first-writer-wins claims), `gate-waiter` (the foreground waiter), `gate-prelude` (the always-logs autonomy ladder), `auto-policy` (R1–R4), `veto-updater` (revision round).
...
- `afk-runner/src/run.ts` — the orchestrator replacement: start/resume/status over the drive loop, the foreground gate-waiter continuation after a gate-pending park (settle → re-drive, holder alive while waiting), the derived `state.json` memo.
- `afk-runner/src/cli.ts` — `start <taskFile>`, `status <runId>`, `resume <runId>`; a bare run-dir arg keeps printing the C1 fold summary.
- `tests/afk-runner/` — kernel/graph/parity/drive/cli/integration tests, `fixtures/fake-pipeline.ts` (stubbed-agent harness), plus `fixtures/real/` (10 historical runs) and `fixtures/scenarios/` (5 extracted shapes, the veto scenario synthetic-marked) that the golden-replay harness folds.

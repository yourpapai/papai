...
- `src/bot-message-handler.ts` — authorized message handling: context seeding, attachments, mid-run steering, queueing, and the queued-turn lifecycle (analytics turn facts, reply delivery tracking).
...
- `src/analytics/` — privacy-gated analytics (four lanes, kill switch, source→normalize→govern→store→derive→snapshot/egress flow): fact contracts + closed registry, normalizer/runtime, turn/LLM/tool/feature instrumentation, governance (policy/preferences/eligibility/grants/subject rights), identity keyrings + pseudonyms + storage generations, delivery outbox (nine-state ledger, write-only sink lifecycle, DNS-pinned transport, thresholded aggregate releases), derive materializations (sessions/outcomes/intent/features/friction), governed usage backfill + reconciliation, retention/expiry, rekey workflow with cutover fence, curated Metabase snapshot publisher + fail-closed consumer coordinator, bounded lifecycle jobs, and executable rollout stage gates (`rollout/stage-gates.ts`). Operator docs: `docs/operations/analytics-runbook.md`, `docs/operations/analytics-incident-runbook.md`.
...
- `src/debug/` + `client/{debug,admin,settings}/` — optional debug server and three UIs (below). Inside it, `state-collector.ts` runs a persistent, unfiltered debug-event capture (buffers + stats) subscribed at production-deps creation; SSE fan-out and read-time visibility are gated per the bullet below.

## Debug/settings server surfaces

- `/debug` — engineer live-observability surface; `/admin` — **read-only** operator dashboard (Overview, Billing, Stats, Memos, Reminders, Identities — no admin controls or settings); `/dashboard` → redirects to `/debug`. Admin controls now live solely in the settings admin section (`/settings/api/admin/*`, super-admin gated): LLM providers + roles (`/settings/api/admin/providers`, `/settings/api/admin/llm-roles`), platform/task instances (`/settings/api/admin/platform-instances`, `/settings/api/admin/task-instances`, `/settings/api/admin/admins`; apply via `POST /settings/api/admin/platform-instances/apply`), plugin config (`/settings/api/admin/plugin-config`), group authorize/remove (`/settings/api/admin/groups`). Instance rows with unreadable encrypted config are reported in an `unreadable` array and skipped at startup with warnings, not aborts.
...
- **Persistent event capture, read/broadcast-time visibility:** the debug-event-derived buffers (`recentLlm`, `recentTurns`, `recentNotifications`, `recentToolFailures`, pending traces) and the `stats` counters are captured by a persistent collector in `src/debug/state-collector.ts` that an idempotent `startEventCollector()` subscribes once inside `createProductionRuntimeDeps` — from process start, independent of dashboard connections, runtime start/stop, `DEBUG_SERVER`, and admin identity, and never unsubscribed. Capture is **unfiltered** (non-admin activity lands in the bounded in-memory buffers; totals no longer undercount), while visibility is enforced exactly at the two exposure points: SSE **broadcast** (admin-scope gate + no-op with zero clients; `llm:full` only for `trace.userId === adminUserId`) and **read time** (`state:init` filters turns/notifications/tool failures by stored scope and `recentLlm` by trace `userId`; the REST `/turns/:id` lookup keeps its `isScopeVisibleToCurrentAdmin` 404 contract). `addClient`/`removeClient` manage only the client set and the SSE heartbeat. Buffers stay process-local and volatile (wiped on restart); the log buffer (`/logs`) keeps its own connection-scoped lifecycle.
...
- **Analytics settings routes** (`src/debug/settings/analytics-routes.ts`, `src/debug/settings/admin/analytics-routes.ts`): actor self-service `GET/PUT /settings/api/analytics/preferences` and `POST /settings/api/analytics/{export,withdraw,delete}`; super-admin `GET/PATCH /settings/api/admin/analytics` (policy/governance view + lane toggles), `POST /settings/api/admin/analytics/reconcile` (on-demand reconciliation incl. fail-closed external release assessment), and write-only sink management `POST /settings/api/admin/analytics/sinks` + `POST /settings/api/admin/analytics/sinks/:id/{verify,rotate,disable}`.

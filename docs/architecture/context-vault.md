...
Coding-agent plugin lifecycles are per-session, so watching in-process would fork N watchers and race on push. Instead a thin adapter (`context-vault-indexer/adapters/opencode.ts`) performs a lock-file check on activation and spawns the daemon detached; it never scans or pushes in-process. The lock (`<stateDir>/context-vault-indexer.lock`, `{ pid, heartbeatAt }`) treats a live PID + fresh heartbeat as held, and reclaims on dead PID, expired heartbeat (TTL authoritative over PID liveness), or a corrupt record, via delete + exclusive re-create (`wx`), so concurrent contenders converge on exactly one daemon. The adapter acquires with its own short-lived pid only to serialize concurrent activations, then hands the record off to the spawned daemon's pid (`handoffIndexerLock`); the daemon owns the lock from then on and refreshes `heartbeatAt` on every loop tick — which runs at `min(intervalMs, ttl/2)`, independent of the scan interval (`refreshIndexerHeartbeat` in the `runDaemon` loop), so the lock stays live past the heartbeat TTL and after the plugin process exits. Lock, daemon, adapter, registry, and IPC are all DI-seamed (injected fs/clock/pid/spawn/push/register) and unit-tested under `tests/context-vault-indexer/`; end-to-end behavior is pinned by `SCN-context-vault-push` and `SCN-context-vault-indexer-singleton`. The runnable entrypoint is `context-vault-indexer/main.ts` (`bun run context-vault-indexer:start <stateDir>`), the thin process shell that binds real `node:fs`, `fetch`, `process.env`, and signal handlers to the orchestration in `entry.ts`; that is the `daemonEntry` path the adapter spawns.

## Daemon configuration and multi-repo registration

Settings live in `<stateDir>/config.json` (Zod-validated: `pushUrl`, `intervalMs`, `repos`); the bearer token is read **only** from `CONTEXT_VAULT_TOKEN` and is never written to disk — the daemon rewrites the config file on every registration, so a token stored there would be re-serialized on an uncontrolled schedule, and a `token` key in the file is stripped by parsing. A missing token or an absent/unparseable/invalid config exits non-zero rather than degrading into a loop that indexes nothing or pushes to a default URL.

One daemon serves N repositories. A coding session that finds the lock held does not no-op: it registers its repository with the running daemon over a unix socket at `<stateDir>/context-vault-indexer.sock` (newline-delimited JSON, Zod-validated, 64 KiB cap, `register` and `status` ops), and the registration takes effect on the next scan tick and persists to the config file. The socket is `chmod 0600` and the state dir is forced to `0700` (tightened with a warning when it pre-existed looser) — Bun creates the socket file `0755`, so that chmod is load-bearing rather than defensive. There is no authentication on the socket beyond those permissions, deliberately: a local process able to forge a registration could equally read `CONTEXT_VAULT_TOKEN` from the daemon's environment. Stale sockets are unlinked before binding, but only once the singleton lock is held, which is what proves no live daemon owns the path.

`runDaemon` (in `loop.ts`) takes a repo provider (`() => RepoRuntime[]`) and snapshots it at each tick, scanning repos under `p-limit` at concurrency 4 with per-repo `DaemonFs` bound to `state-<hash>.json`; one repo whose spec dir vanished does not cost the others their scan. Registration resolves a spec dir to its owning repository by reading `.git` (a worktree's `.git` file carries `gitdir: …/worktrees/<name>`) — never by shelling out to `git`, since the daemon is spawned detached with no dependable PATH — so N worktrees of one project stay one vault entry. Registering a different worktree of a known repository re-points its active spec dir; because the state key follows the repo identity rather than the path, that re-point diffs against the same ledger instead of re-pushing everything as new.

Operator-facing setup (approve, enable, mint a token, configure, start, verify) lives in [`plugins/context-vault/README.md`](../../plugins/context-vault/README.md).

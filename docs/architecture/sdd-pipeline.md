...
- **Intake**: depth classification (S/M/L), change scaffolding via `openspec new change`. The read-only estimator reports structural observations; the **runner** computes the oversize verdict (see Composite runs) and every routing decision is evented.

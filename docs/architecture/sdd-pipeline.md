...
- **Intake**: depth classification (S/M/L), change scaffolding via `openspec new change`. The read-only estimator reports structural observations; the **runner** computes the oversize verdict (see Composite runs) and every routing decision is evented. Two classification caveats are surfaced as `intake:` warn lines rather than left in the event log: a `--depth` override skips the estimator, which is the only source of the signals the verdict is computed from, so such a run is never decomposed into child runs; and a two-level split between the estimator and the keyword prescreen names both readings and the higher one taken.
...
Network/parsing failures are swallowed: the resolver returns `null` for every model, so the gate degrades to `$0.00 · <walls>s · unknown` rather than halting. Pricing never fails a run, but on a **metered** run it is not cosmetic either: `costKnown: false` makes the R4 rung gate, so an unreachable database leaves the autonomy ladder inert and every gate human-decided (see Known limitations). The live renderer's status line applies the same resolver at display time (see Live rendering) with a `~$` marker for estimated figures; only the gate digest shows the aggregate (repriced) cost with the full tristate marker.
...
Depth is orthogonal to **routing**: an oversize verdict does not change S/M/L, it changes *which pipeline shape* runs (single change vs parent/child decomposition). `--depth <S|M|L>` pins depth and skips planning entirely; `--plan` forces the plan branch regardless of the estimator's verdict (the two flags conflict loudly).
...
bun run sdd-runner:start -- analyze [workdirs…] [--json] [--config <path>]
...
The `backend` key selects which CLI drives every stage agent: `"opencode"` (the default, and byte-identical to what it always was — same argv, inherited environment, no stdin) or `"claude"`, the operator-installed `claude` CLI in print mode. Nothing else about the pipeline changes shape: same stages, same events, same gate, same ledger, same budget guard. Rollback is deleting the key.

**The credential spelling picks the invocation profile — there is no knob.** Exactly one of `ANTHROPIC_API_KEY` (**bare** profile: `--bare`) or `CLAUDE_CODE_OAUTH_TOKEN` (**native** profile: `--setting-sources '' --strict-mcp-config --mcp-config <empty document>`, both flags on every invocation, killing repo-skill discovery, settings-file loads and `.mcp.json` auto-connect) must be exported, and `LLM_API_KEY` — the opencode gateway route's carrier — must be unset. Both Anthropic spellings set, neither set, or a set `LLM_API_KEY` fails at run entry with a code-prefixed `[CLAUDE_CREDENTIALS]` / `[LLM_CREDENTIALS]` message, **before the run directory exists and before any spawn**, so a mis-set environment costs nothing and leaves no run id to resume. The guard is lazy on purpose: the read-only verbs (`stop`, `analyze`, a report) never spend, so they run on a claude-route config with no credential set at all.

**Per-role allowlists** (`review-loop/src/claude-argv.ts`, shared with the review loop and pinned equal to the parent route's sets by test). The artifact writers — `drafter-*`, `resolver-*` (`drift` and `veto-updater` spawn under that same role), `decomposer`, `atomicity` — get `Read,Edit,Write,Glob,Grep`. The analysts — `reviewer-*`, `skeptic-*`, `estimator`, `planner` — get `Read,Glob,Grep` plus `Write(<cwd>/.review-loop/**)` for their sidecar and nothing else. No runner role gets `Bash`, no set carries a wildcard tool entry, and an unmapped label inherits the analysis (weakest) set with the condition logged. The allowlist is the *second* fence: the post-spawn `guardWorkingTree` check is the real one and is unchanged, failing any stage that touched files outside the change directory.

**The `model` value keeps its `provider/model` prefix in config on both routes.** The CLI is handed the stripped model id; the ledger and the pricing table are keyed by the configured (prefixed) one, so the cost digest reads the same on either route. A claude turn's `result` line carries its own `total_cost_usd` and four token buckets (input, output, cache read, cache creation, folded once into the `done` event, cache counters kept separate); a reported cost survives repricing untouched, and a zero-cost turn reprices from the token buckets exactly like any other (see Cost fallback).

**Install the pinned CLI yourself.** `claude` is an operator-installed binary, not a dependency of this repo and deliberately not installed in CI — nothing about this route is exercised there, since CI has neither a credential nor the CLI. Composition, credential guard, allowlist mapping and stream decode are all pinned by unit tests over injected seams; end-to-end proof is a single credentialed manual run recorded in the change folder.

Trade-offs, carried over from the shared route: there is **no retry layer** beyond the standing retry-once-on-stall, so a transient provider failure (a 429 on a subscription window included) is an ordinary turn failure; a turn **killed before its `result` line contributes no usage**, so token totals under-count — never over-count, which keeps the budget ceiling conservative in the operator's favour; and there is **no session continuation**, because each spawn's `CLAUDE_CONFIG_DIR` is ephemeral, leaving no earlier session addressable by `--resume` — a resumed stage re-spawns from a rebuilt prompt (the fallback is reported as a retrying signal) rather than continuing its prior session, costing one extra spawn the budget guard sees like any other. Each spawn gets its own `CLAUDE_CONFIG_DIR` child under one OS-tmp parent, opened at the first run-driving verb and removed at teardown; removal is best-effort, so a SIGKILL can leak the parent — under `os.tmpdir()`, never inside the checkout, so no commit can stage it.

## Deadline

The config `deadline` key (minutes) arms `gateDeadlineAt` at presentation (bell/notification line; the process exits without blocking). The derived wait: a deadline is set and the context is non-TTY ⇒ the waiter runs. It polls `state.json` and the gate file each second without caching, settles stable hand edits through the normal path (content hash unchanged 3 consecutive ticks), translates landing steer directives, and at expiry claims the gate through the same first-writer-wins settle claim as the TUI, re-runs the ladder conservatively over the run's **real** gate state (sidecar review result, replayed trajectory, gathered cost signals — the same `runPolicyLadder` the prelude uses, metered R4 treatment included; R1 approve, else R2 extend, else stay pending), and re-arms at most once — never auto-aborts. Every claimed expiry outcome appends the standard `auto_decision` event after its write (settle names the deciding rule; re-arm/stay-pending record `none`/`pending`), so replay alone distinguishes waiter-settled gates from human-settled ones; externally-settled gates (hand edit, steer) emit nothing.
...
`analyze [workdirs…]` (`sdd-runner/src/analyze*.ts`) is the read-only reader over retained run artifacts: it loads every run under the given workdirs (default: the config's), replays `events.ndjson` through the one fold (`replay.ts`), joins the sidecars (`findings-*`/`findings-skeptic-*`/`resolutions-*`), gate files, expiry claims, and persisted state, then prints a per-run + corpus report (plain text, no ANSI; `--json` for the same structure machine-readably). **Read-only by construction**: the injected fs seam is `{ readFile, readdir, stat }` — no write member exists on the type (test-pinned) — and git goes through a wrapper that admits only `log`/`ls-tree`; the invocation never routes into a run, gate, or resume flow and cannot disturb pending gates. Old or partial runs load degraded-gracefully: every metric reports either a value or an explicit `unknown` with its reason, never a failure.

Metric inventory (each a named pure function over the fold + sidecar joins): trajectory (per-round class counts, verdicts, resolved/dismissed); gate forensics (presented→answered latency with human/policy/waiter attribution — a waiter settle is an `auto_decision` with a matching `expiry-claim` — never-answered gates with age, extends by origin, auto-decision rules fired); `duplicateIdRate` (ids repeated within one round's resolutions ledger — the dead-dedup signature); `lensOverlapRate` (skeptic findings whose `fingerprintOf(gap)` matches a same-round reviewer finding); `classChurn` (multi-round ids whose class changed); `resolverActionMix`; `concernPersistence` (fingerprinted concern clusters spanning ≥2 rounds); `r2EligibilityRate` (cap-hit convergence pairs satisfying the exact R2 predicate); retry taxonomy (stall vs validation per role); per-role usage through the reprice seam (`costKnown` fail-closed). The **decision-record consistency audit** cross-checks the three writers (event log, gate files, `state.json`): answered-without-presented versions, completion after an ABORT no later presented-and-answered chain supersedes, `.bak` residue, and decision-carrying gate files with no answered event each flag; any phantom-answer or post-abort-completion signature marks the run **era-contaminated**, and corpus aggregates exclude such runs (they stay visible per-run with the flag). The **ground-truth join** maps each run's change folder to openspec + git — tasks done/total, folder existence, commits on the current branch, presence on a main ref (`main`/`master` via `ls-tree`) — surfacing `stranded-complete` (planning done, unmerged) and `merged-unimplemented` (on a main ref, zero tasks done) as report sections.

## Composite runs

An oversize task takes a decomposed path end to end. The verdict is **signal-grounded, not self-declared**: the estimator prompt asks only for observations (`implicated_files`, the signal record, `rationale`), and `runIntake` computes `oversize` deterministically — `novelty: new-subsystem` AND `cross_module: true` AND implicated-file count ≥ `OVERSIZE_MIN_IMPLICATED_FILES` (compiled at 30; corpus-calibrated so the knowledge-base run's 36 files route while the 19-file near-miss stays single-path). Any agent-emitted `oversize` boolean is overridden by the computed verdict, and the depth sidecar records the weighed `oversize_signals` (`{ novelty, cross_module, implicatedFiles }`) so every routing decision is auditable and the threshold can be recalibrated from real sidecars. The `depth` event carries the verdict, the signals, and whether the route was estimator-decided or operator-forced (`routeForced: 'plan'` from `--plan`, `'depth'` from an explicit depth override); replay folds them into `ReplayState.routing`, so routing rates are measurable from `events.ndjson` alone. Plan-branch machinery (planner, plan gate, child runs) is owned by the `sdd-runner-decomposition` change; this routing verdict is the intake handoff into it — an oversize verdict skips the scaffold entirely, recorded **before any scaffolding**, and `--plan` reaches the same branch with no scaffold when the operator forces it. In the single branch `driver.newChange` moves after the estimator, so folder contents are identical and a failed estimator leaves nothing behind. On routing, the planner (`runPlanner`, role `planner`, `PlanSchema` sidecar at `sidecars/plan.json`) drafts a plan, and structural failures (duplicate ids / unknown deps / cycles from `validatePlan`/`topoSortChildren`) trigger exactly one replan pass with the validation error appended to the prompt (bounded by `PLAN_REPLAN_PASSES`), then a loud failure. `runPlanBranch` (`children.ts`) materializes the child task files, emits the `plan` event, records `state.plan` + seeds `state.children`, and presents the **plan gate** — one `C<n>` checkbox row per child in topo order, membership-routed by the parser like assumptions/findings; every C-box checked approves, an unchecked C-box vetoes that child with an optional `→ <redirect>`, `ABORT` aborts, and `→ RUN 1 MORE` is rejected (cap-hit only). The plan gate's prelude is **R4 only**: the budget guard runs with the per-child projection `spent + childCount × DEFAULT_ROUND_COST_USD` (plus the tree baseline), no rule can approve/extend/accept-items there, and the auto-settle/extend paths refuse plan mode loudly.

Approval drives `runChildren` (`children.ts`): a sequential walk of `state.plan.childIds` — one child in flight, skipping children already `done` — where each child is a nested `runStart` over its materialized task file. The loop emits `child_spawned { child, runId }` then `child_done { child, outcome, usage }` (usage aggregated from the child's own log), surfaces a gate-pending child's concrete `sdd <childRunId>` line and records it `running` (the operator settles the child's gate, then resumes the parent, which skips forward), and stops immediately on a child ending aborted/failed/stopped — `child_done 'failed'`, parent `stopped` with an operator line naming the blocking child (an unloadable child state counts as not-done, fail closed); when the operator later completes that failed child's run and the parent resumes, the flight is adopted as `done` and a fresh `child_done 'done'` with the full current usage supersedes the stale failure-time line. The parent marks `completed` exactly when every child reads `done`, and **never creates a change folder** — only children own one. The **tree budget ledger fails closed**: `treeSpend` = parent done-events + Σ per-flight `child_done.usage` (last `child_done` per spawn-to-spawn flight, so an outcome flip counts once while a retried child's new flight spends again) with `costKnown: false` when any usage is absent; the loop halts before each `child_spawned` on unknown-or-exceeded spend (parent `stopped`, loud line naming the budget guard), and the parent passes its committed spend as a `spendBaselineUsd` into each nested run so the child's R4 guard compares parent + prior children against the single budget. The agent backend’s own run account rides the same fail-closed rule: opencode’s `usage()` sums the session tree with absent-is-not-zero — a cache bucket any session omits leaves the tree unpriced rather than under-priced — and degrades to the session’s own figure when the tree cannot be read whole. **Calm-stop is subtree-scoped**: while a child is in flight the parent watches its own marker and writes the *child's* stop marker on request; the child honors it at its next boundary, the parent consumes its own marker and settles through the shared stopped tail — both nodes resumable, completed children untouched. A plan-gate **veto round** (`settlePlanVeto`, `gate-resume-tail.ts`) re-runs the planner with the round's redirects, re-materializes wholesale, emits a fresh `plan` event (the replay fold resets `children`), and re-presents at `gate-<n+1>.md` with the policy ladder skipped — rounds repeat until approve or ABORT, the only terminals. Parent resume intercepts plan-carrying states before the single-run resume flow and re-drives `runChildren`.
...
**An unreachable pricing DB disables the autonomy ladder on a metered run.**  `loadDb` (`pricing.ts:187`) swallows every fetch/parse failure and answers `{}` when no cache exists, so `resolveCost` returns `null` for every model, `costKnown` goes false, and `r4FailsClosed` — a never-cut pre-check ahead of R1/R2/R3 — gates. Net effect on a metered run whose host cannot reach `models.dev`: no gate is ever auto-decided, the ladder is inert, and every run waits for a human. Reproducible in any network-isolated container. *Narrowed by `sdd-policy-metered-budget`, merged after this entry was written:* the cost-unknown branch now predicates on `signals.config.metered`, so an unmetered run (`budget: null` / `metered: false`) is no longer vetoed for unknown cost and its ladder stays live — the round cap and the R2 trajectory carry that spend risk instead. The metered case is unchanged. Failing closed is still the right direction, but the `unknown` marker (`gate-render.ts:36`) is emitted both when pricing is unreachable and when agents genuinely reported no usage, so an operator cannot tell an inert ladder from a free run. **Revisit** by separating those two causes in the marker; that is the whole fix.

**A gate deadline auto-settles only since the metered-budget merge; `deadlineExpired` is still dead.** As written, `conservativeBranchApplies` re-ran the ladder at expiry with a synthetic signals object hardcoding `costKnown: false`; since R4 fail-closed precedes R1/R2 the ladder always answered `gate`, the predicate always returned false, and an expired deadline re-armed once then stayed pending indefinitely. *Superseded by `sdd-policy-metered-budget`, merged after this entry was written:* `conservativeBranchApplies` (`deadline-waiter.ts:245`) now reads the run's real sidecar review result and gathers real signals through `gatherGateSignals`, so R1 approve and R2 extend can genuinely fire at expiry. The remaining half still holds: `deadlineExpired` is never set `true` anywhere in `src/` (`gate-prelude.ts:66,102` hardcode `false`), so its one reader — R3 suppression at expiry, `auto-policy.ts:229` — is unreachable. That half was covered by a green test that never met the code: `deadline-waiter.test.ts` injects the branch predicate directly instead of calling the real one. **Revisit when** R3 suppression at expiry is actually wanted; the auto-settle half no longer needs revisiting.

...
Seven fixes remain after reconciling with the branch, validating Run 1, and folding in selected retrospective items — correctness bugs (#1–#4), a cost control (#5), a hardening fix (#6), and a process/schema change (#7). Each maps to existing tests under `tests/review-loop/`.
...
**6. No SIGKILL escalation on timeout** (`cli.ts:119-125` `realSpawn`). On timeout the loop sends `SIGTERM` and resolves, but a child that ignores `SIGTERM` hangs until the promise resolves anyway; a truly stuck child can outlive the grace period. **Fix (hardening):** after `SIGTERM`, start a short grace timer (e.g. 5s, configurable); if the child has not exited, `child.kill('SIGKILL')`. Resolve with the timeout error regardless. Pure `realSpawn` change; the existing timeout tests gain a SIGKILL-after-grace assertion.

**7. Plan/spec edits silently laundered as `fix(review-loop)`** (`issue-schema.ts:26` verdict enum; Run 1 `a44ea54cb`). Run 1 showed the fixer resolve a "plan divergence" issue by **rewriting the source-of-truth spec** in a normal fix commit — the document drifted to match the code instead of the reverse. **Fix (process/schema):** add a `plan_drift` value to the `VerifierDecisionSchema` verdict enum (additive — old results predate it). When an issue is really "the code diverged from the plan/spec but is not a code defect" (extra files, different structure, scope expansion), the fixer returns `verdict: plan_drift` with rationale describing the divergence and **does not edit any file**. `mapVerifierDecisionToLedgerStatus` maps `plan_drift` → `needs_human` (terminal) so it surfaces for a human and stops consuming rounds. The fixer prompt gets an explicit clause (see Section 3). `plan_drift` flows through the trace's `verify_complete` event unchanged.
...
- `loop-controller.test.ts`:
  ...
  - **SIGKILL escalation (#6):** a fake child that ignores `SIGTERM` is sent `SIGKILL` after the grace period and the spawn still resolves with the timeout error (test `realSpawn` via an injected stubborn child process).
  - **plan_drift verdict (#7):** a fixer result `{verdict:'plan_drift', ...}` lands the issue as `needs_human` (terminal) and is not re-processed next round; `VerifierDecisionSchema` accepts `plan_drift` and still accepts the pre-existing verdicts (additive).

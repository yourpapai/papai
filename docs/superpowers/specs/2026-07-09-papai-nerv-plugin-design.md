...
| `list_coding_tasks`    | local records + `GET /tasks/:id`                                | Lists the group's `TaskRecord`s, enriched with live status/cost via `Promise.all` GETs (nerv has no list endpoint; unbounded over the group's small record set — plugin code cannot import p-limit and a hand-rolled pool trips oxlint's no-await-in-loop, matching acp's answer_permission precedent). |
...
- **Tool executors** with a mock `httpFetch` + mock `codingRepos`:
  ...
  - `coding_task_status`/`list_coding_tasks`: enrichment via `GET /tasks/:id`, `usageUsd`
    surfaced, unbounded `Promise.all` fan-out (see §8).

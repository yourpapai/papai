...
- `checkAuthorizationExtended()` receives `platformInstanceId` from `IncomingMessage.platformInstanceId` / `IncomingInteraction.platformInstanceId`; no Phase 4 auth path reads `CHAT_PROVIDER`
...
`users.platform_instance_id` is part of this auth predicate. Migration 040 already adds the column, but Phase 4 must update `src/db/schema.ts`, `src/users.ts`, and existing callers so authorization is scoped to the message's source platform instance. Legacy rows with `platform_instance_id IS NULL` are not considered authorized after Phase 4; bootstrap and commands must write explicit platform IDs.

### Bootstrap source

`ADMIN_USER_ID` from env, processed during Phase 1 bootstrap, is the seed for both `admins(__super__)` and `admins(<bootstrapped-platform-id>)`. Runtime admin authorization reads `admins`, not `ADMIN_USER_ID`; the env var remains required at startup only for first-run bootstrap and dashboard write attribution where no session identity exists.
...
- `/users` lists rows for `msg.platformInstanceId` when run by a platform-admin; super-admins may still see all rows for operator diagnostics
...
| `enable`, `disable` | DM-only, any admin authorized for the target context-id's `context_settings.platform_instance_id` |
...
All forms POST to the API in Section 4. Configs are entered as key/value pairs and displayed with secret-masking from the server. The client must not mask secrets itself; it renders only the masked config returned by the server.
...
POST   /api/platform-instances/:id/status  { status }
...
- DELETE returns `204`
- POST validates `id`, `type`, `status`, and `config` against a Zod schema; secret values are encrypted via Phase 1's helper before insert
- `POST /api/platform-instances/:id/status` supports Start / Stop by setting `status` to `active` / `stopped`; it does not start or stop the live adapter until Apply is pressed
...
Phase 4 must add a readonly `ChatRouter.listInstances()` method because Phase 3 currently exposes `getInstance()` but not a list. If the router has not been initialized (e.g., tests or a brand-new debug-only process) the endpoint returns `503 { error: 'router not initialised' }`. The dashboard renders that as "Apply needs the bot to be running with `bun start`."
...
- **`tests/users.test.ts`** — per-platform `isAuthorized`, super-admin shortcut, username resolution scoped by platform instance
- **`tests/auth.test.ts`** — `checkAuthorizationExtended` uses `platformInstanceId` and stops relying on `ADMIN_USER_ID` for runtime admin checks
- **`tests/commands/admin.test.ts`** — `/user add` writes `platform_instance_id`, `/user remove` only removes rows from the source platform, platform-admin vs super-admin listing behavior
...
- **`tests/chat/router.test.ts`** — readonly `listInstances()` for Apply reconciliation
- **`tests/debug/instance-routes.test.ts`** — CRUD coverage, masked-secret response shape, 400 on schema violation, 401 on write without configured token, 503 on apply-without-router

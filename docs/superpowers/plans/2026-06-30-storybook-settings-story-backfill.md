...
- **`client/stories/**`is excluded** from knip and the production bundle, and`.stories.svelte`files are not bundled — so these changes do not affect`bun knip`or`bun check:bundle-isolation`. `bun test`ignores`tests/visual/\*\*`.

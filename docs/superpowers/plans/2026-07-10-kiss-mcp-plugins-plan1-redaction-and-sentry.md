...
Captured at closeout of Plan 1 (redaction foundation + `mcp-sentry`), for whoever picks up Plans 2-9:

- **`SentryClient.request` does not thread `abortSignal`** (`plugins/mcp-sentry/client.ts`) — it never forwards a caller-supplied `AbortSignal` into the `httpFetch` call, so an in-flight Sentry request cannot be cancelled from `callPluginMcpTool`'s `options.abortSignal`. Add `abortSignal` support to `request()` (and thread it from `index.ts`'s tool `execute` functions) as a follow-up.
- **Per-plugin/per-tool redaction prompt override is still deferred.** Plan 1 ships only the shared `DEFAULT_REDACTION_PROMPT` (`src/mcp-server/redaction.ts`). The roadmap appendix above already flags this for `mcp-youtrack`'s attachment-content redaction; implement it as a context-scoped `redaction_prompt` config key read via `runtimeContext.contextConfig.get('redaction_prompt')`, falling back to the default, and thread the resolved prompt into `redactText`.
- **Schema tests should assert the schema contract structurally, not via a validator.** papai does not locally validate plugin MCP tool inputs against the declared JSON schema at call time (validation, if any, happens upstream/agent-side) — `getPluginToolInputSchema` just forwards the raw schema for `asSchema().jsonSchema` derivation. `tests/plugins/mcp-sentry-schema.test.ts` asserts each tool's schema object structurally (required keys, types, `additionalProperties: false`) rather than round-tripping through a bespoke runtime validator. Future MCP-plugin plans (2-9) should follow that pattern rather than reintroducing a validator.
- **The Svelte settings-UI panel for `mcp_redaction` is deferred.** The backend admin-config API exists (`src/debug/settings/admin/mcp-redaction-routes.ts`, `src/coding-credentials/mcp-redaction.ts`), but there is no dedicated settings-UI panel yet for operators to configure `model_url`/`api_key`/`model_name`/`timeout_ms` — it currently has to be set through the raw admin-config route. Add the panel alongside the next plugin plan that needs redaction configured through the UI.

### Deferred low-severity code-review items (final review)

The final whole-implementation review raised two LOW items intentionally deferred (all Critical/High/Medium findings were fixed on this branch — see commits `e53413547`, `48c420568`, `bdc51a0b7`):

- **`mcp_redaction` has no unset/clear operation.** `src/debug/settings/admin/mcp-redaction-routes.ts` exposes only `GET`/`PUT`, and `src/coding-credentials/mcp-redaction.ts` has no clear function — an operator cannot roll back to "unconfigured" (which would re-disable redacting plugins via the fail-closed eligibility guard). Add a `DELETE` route + a `clearMcpRedactionConfig` helper when the settings-UI panel lands.
- **Dead `key === 'key'` carve-out in the static sanitizer** (`plugins/mcp-sentry/format.ts`) — the `SECRET_KEY` regex never matches a bare `key`, so the explicit exemption is a no-op today (harmless, ported faithfully from kiss). Remove it, or document intent, so a future regex tightening can't silently reintroduce a bypass.

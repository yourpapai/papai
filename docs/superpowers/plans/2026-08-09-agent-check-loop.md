...
- [x] **Step 1: ~~Raise the PostToolUse timeout~~ — withdrawn, the premise was wrong.** This step read `.claude/settings.json`'s `"timeout": 200` as milliseconds and concluded a ~3 s targeted `bun test` would be killed before reporting. The field is **seconds**. The Stop hook's `"timeout": 300` sits beside `execFileSync(..., { timeout: 300_000 })` in `check-full.mjs`, which pins the unit. 200 s is ample; nothing to change. Kept in the plan rather than deleted because the underlying worry — a hook killed at its timeout looks exactly like a hook that passed — is sound, and the next person to touch these files should see it was checked.

...
3. Test seams use **alias-const shims**, not bare re-exports: knip 6.14.1 does not register usage through `export { x } from './m.js'` when the re-export is itself unconsumed, so shims use `import { x as _x } from './m.js'; export const x = _x`. Identity tests (`tests/**/*.testing.test.ts`) assert each shim binding is reference-equal to the production binding. Seams like `resetNotifyTokenCacheForTesting` mutate module-private state and cannot move without exporting that state, so production modules keep the symbols; one `ignoreIssues` glob covers all shims.

...
| 6. Accessibility                | pass  | Real `<button>` with `:focus-visible`; the toggle now carries `aria-describedby` naming the state `Pill` and help caption, the load-error text has `role="alert"` (`:96`), and the toggle-mutation error banner now also carries `role="alert"`. `aria-pressed` is deliberately absent — see the finding below. |

...
**States captured:** AggregateDefault, MixedPreferences, Error, Loading, WithdrawalInProgress, destructive-action hover, first-Tab keyboard focus, withdraw confirmation, delete confirmation, post-deletion success, failed preference save · desktop (1280) + ~640px narrow, incl. narrow confirmation dialog
...
**States not captured:** `subjectRightsAvailable: false` — the state in which three of the
four controls disable — has no fixture in `client/stories/msw/settings-handlers-personal-2.ts`,
and adding one is outside this skill's write scope. Findings about that state are read from
source only. The section also takes no props, yet its stories pass `contextId` / `scope` args
(`AnalyticsPreferencesSection.stories.svelte:16`) — story hygiene, not a UX defect.
...
- **Suggested fix:** the row gap is a literal `12px` rather than `--gap-inline`, the inter-field margin uses `--gap-inline` (12px) where the field rhythm token is `--gap-field` (20px), and the 13px label sits on no rung of the type scale (`Field` labels are 10px, captions 11–12px).

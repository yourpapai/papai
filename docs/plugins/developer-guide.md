...
| `mcpServer`                          | When `true`, exposes this plugin's registered tools as an MCP server surface at `/mcp/plugin/<pluginId>` for the coding agent to consume.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `mcpResponseRedaction`               | When `true`, this plugin's MCP tool responses are run through the bridge-level redactor before leaving papai. Requires operator `mcp_redaction` admin config; fail-closed otherwise.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
const baseUrl =
  runtimeContext.contextConfig.get('base_url') ??
  runtimeContext.adminConfig.get('base_url') ??
  'https://api.example.com'
```

Context-scoped `api_key` and `model` let individual users or groups bring their own credentials (BYOK). Context-scoped `base_url` is also overridable, but must be **strictly paired** with context `api_key`: if only one of `base_url` or `api_key` is set in context config, return an `incomplete_context_override` error rather than mixing a context endpoint with the admin key (or vice versa). If neither is set, fall back to the admin pair. Admin-config-sourced hosts bypass the HTTPS + public-IP validation (operator-trusted tier); context-config-sourced hosts pass the allowlist membership check but require full HTTPS + public-IP validation (untrusted tier).

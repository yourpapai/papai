...
| Durable state     | `plugin_admin_state.state` holds only `approved`, `discovered`, or `rejected`. `active`, `error`, and `incompatible` are recomputed in-process on each startup. Legacy unknown persisted states are normalized from approval metadata during startup. |
...
| Permissions (MVP) | `storage`, `tasks.read`, `tasks.write`, `commands`, `scheduler`, `provider.task`, `identity`, and `http`. Runtime gating exists for these declared permissions; raw chat sending is not exposed.                                                      |

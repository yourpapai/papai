...
| Claim consumption    | `GET /auth/claim?n=<nonce>` validates the nonce, marks it consumed (single-use), mints a session, and redirects to `/admin` with a `Set-Cookie`.       |
...
| Route surface        | `GET /auth/claim`, `POST /auth/logout`, `GET /auth/whoami` — all mounted before the session cookie gate in the debug server.                           |

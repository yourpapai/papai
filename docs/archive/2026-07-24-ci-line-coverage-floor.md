...
Three claims in this plan, as originally written, do not hold and are corrected here rather than rewritten in place:

- **The plan's core mechanism is wrong: bun's `coverageThreshold` cannot enforce an aggregate floor.** It is a **per-file** rule — every file must individually clear the bar — so the plan's design (set the key to an aggregate baseline, let bun gate) can never pass on this repo: 202 of 1108 covered files are below 90% lines, two at 0.00%. Measured on bun 1.3.13 with a fixture whose worst file is 54.55%, pooled 58.33%, and reported mean 77.27%: threshold `0.50` exits 0, `0.56` exits **1**. 0.56 is above the per-file minimum but below both aggregate metrics, so only a per-file rule explains it. The `All files` row bun's text reporter prints is the unweighted per-file mean and has no bearing on the gate; the gate also fails silently, with no output naming coverage as the cause. **Resolution (implemented):** `coverageThreshold` was removed from `bunfig.toml`, the floor moved to `scripts/coverage/floor.json`, and `scripts/check.sh` now gates on `bun coverage:ratchet` after a passing coverage run. Task 1's `parseBunfigThreshold`/`applyThreshold` became `parseFloor`/`serializeFloor` accordingly, and Task 3's bunfig edit is reporter configuration only.

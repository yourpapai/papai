...
> **Resync correction (2026-05-29, applied):** the above authenticated-healthcheck shape is WRONG for the actual call contract. `validateConfig` is invoked by `validateTaskInstanceConfig` (`src/debug/task-provider-type-routes.ts`) with the **task-instance config only** — instance-scoped fields (`baseUrl`, `internalUrl`). `credential` and `workspaceId` are context-scoped (per-user `user_config`) and are **not** present, so an authenticated healthcheck is impossible here and requiring `credential` would make every Kaneo instance creation fail. The shipped `validate-config.ts` instead validates only the instance URL shape (baseUrl present + valid http/https URL) and returns `Promise<{ok}|{ok:false,reason}>`. There is no `KaneoClient` class (only `kaneoFetch`); no `ping()` was added. Credential validation happens during `/setup`.
...
| 2026-05-29 | In-plan, stale approach        | Task 3.5 `validateConfig` (and spec §3.3) assumed an authenticated healthcheck against `baseUrl`+`credential`; but the validator is invoked with **instance-scoped config only** (`validateTaskInstanceConfig`), so `credential`/`workspaceId` are absent — requiring them would break Kaneo instance creation. Also no `KaneoClient` class exists (only `kaneoFetch`).                                             | Shipped `validate-config.ts` validates instance URL shape only (baseUrl present + valid http/https); added a `validate-config.test.ts`; strengthened `activation.test.ts` to drive `activate()` via a typed mock `PluginContext` exercising both credential branches. Plan Task 3.5 Step 5 corrected. (commit `98ca4fd0`)                                                                                                                                       |
| 2026-05-29 | Out-of-plan, discovered (debt) | Moving `provision.ts` into the plugin (Task 3.3, planned) exposed that core (`src/commands/setup.ts`, `src/commands/start.ts`, `src/llm-orchestrator.ts`) imports it — now a **core→plugin** dependency. `maybeProvisionKaneo` is internally gated (no-op unless the context's task instance is active and `type==='kaneo'`), so no functional bug, but the dependency direction is backwards for the plugin model. | **Pre-existing coupling, merely relocated by a planned move; not blocking.** Accepted as transitional. Proper fix (a plugin-contributed provisioning/setup hook so core doesn't import provider code) is a separate design effort — tracked as a follow-on, not addressed in this plan. Surfaced to user at Phase 3 checkpoint.                                                                                                                                 |
| 2026-05-29 | Out-of-plan, on-goal (tooling) | knip `project` scope was `src/`/`client/` only, so src exports consumed solely by moved plugin code (e.g. `clearCachedTools`, used only by `plugins/task-provider-kaneo/provision.ts`) were false-flagged as unused.                                                                                                                                                                                                | Added `plugins/**` to knip `project` and `plugins/*/index.ts` to `entry`; ignore only the dynamically-loaded plugin index default exports (commit `f6a420f3`). **Phase 4 TODO:** when `plugins/task-provider-youtrack/index.ts` is wired (Task 4.4), add `"plugins/task-provider-youtrack/index.ts": ["exports"]` to `knip.jsonc` ignoreIssues. The transitional `"src/users.ts": ["exports"]` ignore stays until Task 3.10 (those helpers are genuinely dead). |
| 2026-05-29 | Minor (follow-up)              | `src/config.ts` `SENSITIVE_KEYS` still lists flat `kaneo_apikey`, not the namespaced credential key. Verified **no leak**: all credential-display paths mask via `field.sensitive` (the kaneo credential field has `sensitive:true`), not the raw-key `SENSITIVE_KEYS` fallback.                                                                                                                                    | Left as-is (no leak). Optional defense-in-depth follow-up: drive key-name masking off descriptor `sensitive` flags rather than a hardcoded key set.                                                                                                                                                                                                                                                                                                             |
| 2026-05-29 | Out-of-plan, discovered (debt) | `src/llm-orchestrator-config.ts:checkRequiredProviderConfig` filters `getConfigKeysForContext` for the literal flat keys `kaneo_apikey`/`youtrack_token`. Post-migration, kaneo's context keys are namespaced, so this pre-check is now a **no-op for kaneo** (still works for the still-builtin youtrack).                                                                                                         | **Not a functional hole** — the resolver (`buildConfigFromDescriptor`) still gates missing required context fields → "needs /setup". Benign degraded pre-check; a pre-existing limitation for ANY contributed provider. Follow-up: generalize `checkRequiredProviderConfig` to read the descriptor's required context fields (namespaced) instead of hardcoded flat ConfigKeys. Not addressed in this plan.                                                     |

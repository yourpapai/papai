...
/**
 * The reviewer/resolver prompt builders (loop-memory D4 extraction —
 * review-model.ts sat at the max-lines edge). The reviewer's previously
 * resolved surface is the capped known-concerns digest, not the flat
 * round-untagged ledger: one round-tagged line per concern, grouped by gap
...
export function buildReviewerPrompt(input: {
  ...
  if (input.ledger.length > 0) parts.push('', '## Known concerns', ...concernDigest(input.ledger))
  const idShape = input.lens === 'skeptic' ? '"id": "S<n>"' : '"id": "F<n>"'
  parts.push(
    '',
    '## Artifacts',
    input.artifacts,
    ...
    `{"findings": [{${idShape}, "class": "BLOCKER"|"MATERIAL"|"NITPICK", "gap": "<verbatim quote>",`,
    ' "question": string, "code_evidence_attempted": string}]}',
    'Do not re-raise a known concern without new evidence.',
  )
  return parts.join('\n')
}
...
export function buildResolverPrompt(input: {
  ...
  return [
    ...
    ' "assumptions": [{"id": "A<number>", "findingId"?: string, "text",',

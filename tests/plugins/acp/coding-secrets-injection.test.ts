...
import { reviewPrTool, startSessionTool } from '../../../plugins/acp/tools.js'
...
function ctx(resolve: () => Record<string, string> | null): FakeRuntimeContext {
...
test('review_pr includes resolved secrets in the POST body', async () => {
  ...
  await tool.execute(
    { project: 'demo', prNumber: 42 },
    ctx((): Record<string, string> => ({ ANTHROPIC_API_KEY: 'sk-2' })),
    {},
  )

...
import { jsonResponse, options, runtimeCtxWithKv } from './support.js'
...
describe('acp continue_session tool', () => {
  test('continues by sessionId: POSTs follow-up and records the child', async () => {
    ...
    const res = asRecord(
      await continueSessionTool(httpFetch).execute(
        { sessionId: 'p1', prompt: 'fix tests' },
  ...
  test('forwards the current thread contextId on the follow-up request', async () => {
    ...
    await continueSessionTool(httpFetch).execute({ sessionId: 'p1', prompt: 'go' }, runtimeCtxWithKv(store), options())
  ...
  test('refuses not_configured when the forge token is missing', async () => {
    ...
    const res = asRecord(await continueSessionTool(httpFetch).execute({ sessionId: 'p1', prompt: 'x' }, ctx, options()))
  ...
  test('resolves a prNumber to a known session id via the done list', async () => {
    ...
    const res = asRecord(
      await continueSessionTool(httpFetch).execute(
        { prNumber: 42, project: 'demo', prompt: 'go' },
        runtimeCtxWithKv(store),
        options(),
      ),
    )

...
import { embedSlugBatch, type EmbedSlugBatchDeps } from '../../../scripts/behavior-audit/consolidate-keywords-agent.js'
...
function makeDeps(embedManyImpl: EmbedSlugBatchDeps['embedMany']): EmbedSlugBatchDeps {
  return {
    embedMany: embedManyImpl,
    buildEmbeddingModel: () => 'mock-embedding-model',
  }
}

test('embedSlugBatch calls embedMany once for inputs within batch size', async () => {
  const calls: string[][] = []
  const deps = makeDeps(({ values }) => {
    calls.push([...values])
    return Promise.resolve({ embeddings: values.map(() => [0.1, 0.2]) })
  })

  const result = await embedSlugBatch(['a: desc a', 'b: desc b'], deps)
...
test('embedSlugBatch splits large input across multiple batches', async () => {
  ...
  const result = await embedSlugBatch(inputs, deps)
...
test('embedSlugBatch returns embeddings in order matching input', async () => {
  const deps = makeDeps(({ values }) => Promise.resolve({ embeddings: values.map((_, i) => [i]) }))

  // batchSize=2, 4 inputs
  const result = await embedSlugBatch(['a', 'b', 'c', 'd'], deps)
...
test('embedSlugBatch throws after exhausting retries', async () => {
  ...
  const deps = makeDeps(() => Promise.reject(new Error('API unavailable')))

  await expect(embedSlugBatch(['a'], deps)).rejects.toThrow('Failed to embed batch')
})

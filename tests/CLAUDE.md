...
- Refactor qualification freezes **every regular file** under `tests/stories/**`; `scripts/story/**`; plus `bunfig.toml`, `tests/setup.ts`, `tests/mock-reset.ts`, `tests/utils/test-helpers.ts`, and `tests/utils/logger-mock.ts` byte-for-byte. The runner hashes and executes the same captured bytes from a read-only temporary snapshot while resolving production `src/` and `plugins/` from the live candidate; it verifies snapshot integrity before and after the child and always removes that snapshot after the child exits or setup fails. The candidate may change production/runtime composition only. Establish and commit all frozen files on master, record the manifest `treeHash` and baseline SHA, rebase the refactor onto it, and run compatibility against that SHA. A ref predating frozen inputs is incompatible and reports them as added. Reports are generated under ignored `reports/stories/` as `manifest.json` and `junit.xml`.

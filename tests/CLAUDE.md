...
- The I/O guard permits exactly one child process: the `tsgo` binary TypeScript 7 spawns to serve a syntax tree, and only from a `@typescript/typescript-<os>-<cpu>` package inside the scenario's execution root. Plugin entry-graph discovery parses through it, so denying it turns every plugin into a discovery error. Every other executable and every other spawn entry point stays denied — `tests/stories/harness/io-guard-probe.ts` pins that a tsgo-shaped path outside the root is still a violation. Do not widen the allowance.

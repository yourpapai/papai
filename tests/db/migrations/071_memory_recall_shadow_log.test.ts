...
import { migration071MemoryRecallShadowLog } from '../../../src/db/migrations/071_memory_recall_shadow_log.js'
import { memoryRecallShadowLog } from '../../../src/db/schema.js'
import { setupTestDb } from '../../utils/test-helpers.js'

const migrationsThrough070 = (): readonly (typeof MIGRATIONS)[number][] => {
  const shadowLogIndex = MIGRATIONS.findIndex((m) => m.id === '071_memory_recall_shadow_log')
  if (shadowLogIndex <= 0) throw new Error('071_memory_recall_shadow_log not found after a prior migration')
...
describe('migration 071 memory recall shadow log', () => {
  test('migration id is 071_memory_recall_shadow_log', () => {
    expect(migration071MemoryRecallShadowLog.id).toBe('071_memory_recall_shadow_log')
  })
  ...
  test('applies on a pre-071 database', () => {
    // Reproduce a real upgrade: migrate a fresh DB only through 070 (the state before this
    // feature shipped), then apply the full set.
    const db = new Database(':memory:')
    runMigrations(db, migrationsThrough070())

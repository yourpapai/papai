...
import { migration074MemoryRecordInjection } from '../../../src/db/migrations/074_memory_record_injection.js'
import { memoryProfiles } from '../../../src/db/schema.js'
import { setupTestDb } from '../../utils/test-helpers.js'

const migrationsThrough073 = (): readonly (typeof MIGRATIONS)[number][] => {
  const injectionIndex = MIGRATIONS.findIndex((m) => m.id === '074_memory_record_injection')
  if (injectionIndex <= 0) throw new Error('074_memory_record_injection not found after a prior migration')
  return MIGRATIONS.slice(0, injectionIndex)
}

describe('migration 074 memory record injection', () => {
  test('migration id is 074_memory_record_injection', () => {
    expect(migration074MemoryRecordInjection.id).toBe('074_memory_record_injection')
  })
  ...
  test('applies on a pre-074 database and defaults pre-existing profiles to inject_records off', () => {
    // Reproduce a real upgrade: migrate a fresh DB only through 073 (the state before this
    // feature shipped), write a profile the way pre-074 code would, then apply the full set.
    const db = new Database(':memory:')
    runMigrations(db, migrationsThrough073())
    ...
    // 074 is now the only pending migration; runMigrations skips the already-applied ones.

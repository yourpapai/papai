...
import { migration072MemoryEmbeddingIdentity } from '../../../src/db/migrations/072_memory_embedding_identity.js'
...
describe('migration 072 embedding identity', () => {
  test('migration id is 072_memory_embedding_identity', () => {
    expect(migration072MemoryEmbeddingIdentity.id).toBe('072_memory_embedding_identity')
  })
  ...
  test('stamps pre-existing embeddings as "unknown" but leaves embedding-less rows null', async () => {
    ...
    // blob with no identity metadata at all (the pre-072 state).
    getDrizzleDb()
      .insert(memoryRecords)
      .values({
        ...baseRow,
    ...
    migration072MemoryEmbeddingIdentity.up(getDrizzleDb().$client)

...
/** A settled expiry decision, the shape `processExpiry`'s settle seam answers with. */
const APPROVED: PolicyDecision = { rule: 'R1', action: 'approve', evidenceDigest: 'd' }
...
describe('waiter guards around the gate field', () => {
  ...
  it('an expired run whose gate vanished exits as lost-claim without writing a claim or settling', async () => {
    ...
    const settle = mock((): Promise<PolicyDecision | null> => Promise.resolve(APPROVED))
  ...
  it('the claim file records the claim instant as an ISO timestamp', async () => {
    ...
    const outcome = await processExpiry(workDir, runId, 10, (): Promise<PolicyDecision | null> =>
...
describe('deadlineStampFor', () => {
  ...
  it('arms the deadline exactly deadlineMinutes ahead of now and announces the instant', () => {
    const stamp = deadlineStampFor({
      ...base,
      autonomy: { level: 'assist', costCeilingUsd: 5, deadlineMinutes: 30, metered: true },
    })
  ...
  it('clears both deadline fields when no deadline is configured', () => {
    const stamp = deadlineStampFor({ ...base, autonomy: { level: 'assist', costCeilingUsd: 5, metered: true } })

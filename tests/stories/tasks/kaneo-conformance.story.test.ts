...
// Runs the shared PARITY_GROUPS contract directly through the real Kaneo
...
// kaneo-real.story.test.ts for that).
//
// Kaneo has no exclusions: every one of the 29 shared groups runs. The domain
// partition below mirrors the module a group is authored in
...
// carries the prefix of the domain it errors on rather than 'error').
const included: readonly ParityGroup[] = PARITY_GROUPS
...
if (new Set(partitioned).size !== partitioned.length) {
  throw new Error(`Kaneo conformance domains overlap: ${partitioned.join(', ')}`)
}
const unpartitioned = included.filter((group) => !partitioned.includes(group.id)).map((group) => group.id)
if (unpartitioned.length > 0) {
  throw new Error(`Kaneo conformance domains omit groups: ${unpartitioned.join(', ')}`)
}
...
async function runDomainScenario(
  ...
  for (const group of groups) {
    ...
    // project; state is shared across groups within one scenario.
...
// Six literal scenario names below (not a loop over DOMAINS with a template
// literal) because the manifest/catalog AST extractor
// (scripts/story/scenarios.ts extractStoryScenarios) requires the scenario
// name to be a TypeScript string literal — a template literal fails
// `ts.isStringLiteral` and throws at manifest build time.

scenario(
  'SCN-kaneo-conformance-tasks: real Kaneo provider satisfies the shared task groups',
  (api) => runDomainScenario(api, DOMAIN_TASKS),
  { realTaskProvider: 'kaneo' },
)

scenario(
  'SCN-kaneo-conformance-search: real Kaneo provider satisfies the shared search groups',
  (api) => runDomainScenario(api, DOMAIN_SEARCH),
  { realTaskProvider: 'kaneo' },
)

scenario(
  'SCN-kaneo-conformance-comments: real Kaneo provider satisfies the shared comment groups',
  (api) => runDomainScenario(api, DOMAIN_COMMENTS),
  { realTaskProvider: 'kaneo' },
)

scenario(
  'SCN-kaneo-conformance-relations: real Kaneo provider satisfies the shared relation groups',
  (api) => runDomainScenario(api, DOMAIN_RELATIONS),
  { realTaskProvider: 'kaneo' },
)

scenario(
  'SCN-kaneo-conformance-projects: real Kaneo provider satisfies the shared project groups',
  (api) => runDomainScenario(api, DOMAIN_PROJECTS),
  { realTaskProvider: 'kaneo' },
)

scenario(
  'SCN-kaneo-conformance-errors: real Kaneo provider satisfies the shared error groups',

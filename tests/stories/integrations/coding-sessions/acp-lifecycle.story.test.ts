...
scenario(
  ...
  async ({ given, when, then, world }) => {
    ...
    then.codingSessions(dm).session('pr-session').matches({ project: 'papai', prNumber: 42 })
    expect(
      world.events
        .all()
...
scenario(
  ...
  async ({ given, when, then, world }) => {
    ...
    given.knownCodingSession(dm, 'local-session', {
      project: 'papai',
    ...
    then.codingSessions(dm).session('local-session').matches({ status: 'active' })
...
scenario(
  ...
  async ({ given, when, then, world }) => {
    ...
    given.knownCodingSession(dm, 'missing-session', recordBefore)
    const magi = createFakeMagi({ http: world.http, events: world.events, baseUrl: MAGI_URL, token: MAGI_TOKEN })
    ...
    then.codingSessions(dm).session('missing-session').equals(recordBefore)

...
const PluginsResponseSchema = z.object({
  plugins: z.array(z.object({ id: z.string(), active: z.boolean(), enabled: z.boolean() })),
})
...
scenario(
  'SCN-settings-api-plugins: plugin config rejects unknown keys and persists an authorized plugin selection',
  async ({ given, when, then, world }) => {
    const alice = given.user('alice')
    const plugin = world.fixtures.approvePlugin()
    const session = await given.settingsSession(alice)
    ...
    const invalid = await when.settingsRequest(session, '/settings/api/plugins/config', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ pluginId: plugin.manifest.id, key: 'not-a-real-key', value: 'ignored' }),
    })
    then.responseStatus(invalid, 422)

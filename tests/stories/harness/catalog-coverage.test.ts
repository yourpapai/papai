...
describe('scenario catalog coverage', () => {
  ...
  test('tracks the executable coverage total', () => {
    expect(catalogCoverage.filter((coverage) => coverage.kind === 'executable')).toHaveLength(49)
  })
  ...
  test('audit records cover exactly the pending scenarios', () => {
    ...
    expect(pendingIds).toHaveLength(79)
  ...
  test('audit readiness totals match the audit outcome', () => {
    ...
    expect(states.filter((state) => state === 'executable-as-is')).toHaveLength(5)
    expect(states.filter((state) => state === 'needs-seam')).toHaveLength(52)

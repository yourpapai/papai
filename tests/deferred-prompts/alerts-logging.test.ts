...
// alerts.ts binds `const log = logger.child(...)` at module load, so we
...
type AlertsModule = typeof import('../../src/deferred-prompts/alerts.js')

const importAlerts = (): Promise<AlertsModule> =>
  import(`../../src/deferred-prompts/alerts.js?test=${crypto.randomUUID()}`)
...
describe('alerts logging', () => {
  test('updateAlertActivityState logs the cursor entry and the state update', async () => {
    const { updateAlertActivityState } = await importAlerts()

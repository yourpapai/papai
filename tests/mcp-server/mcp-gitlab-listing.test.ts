...
function discoverMcpGitLab(): DiscoveredPlugin {
  ...
  const plugin = result.plugins.find((candidate) => candidate.manifest.id === 'mcp-gitlab')
  if (plugin === undefined) throw new Error('expected mcp-gitlab to be discovered under plugins/')
  return plugin
}

// Proves mcp-gitlab's tools are reachable through the real end-to-end path an MCP client
...
describe('mcp-gitlab reachable through the mcp-server plugin bridge', () => {
  beforeEach(async () => {
    mockLogger()
    await setupTestDb()
  ...
  test('activation exposes all 5 gitlab tools with object input schemas via listPluginMcpTools', async () => {
    ...
    expect(pluginRegistry.getEntry('mcp-gitlab')?.state).toBe('active')

    const tools = await listPluginMcpTools('mcp-gitlab')

    expect(tools.map((tool) => tool.name).sort()).toEqual([...GITLAB_TOOL_NAMES].sort())
  ...
  test('returns [] once the plugin is deactivated', async () => {
    ...
    expect(await listPluginMcpTools('mcp-gitlab')).toEqual([])
  })
})

...
const RAG_TOOL_NAMES = ['rag_search']

function discoverMcpRag(): DiscoveredPlugin {
  const result = discoverPlugins(join(process.cwd(), 'plugins'))
  expect(result.errors).toEqual([])
  const plugin = result.plugins.find((candidate) => candidate.manifest.id === 'mcp-rag')
  if (plugin === undefined) throw new Error('expected mcp-rag to be discovered under plugins/')
  return plugin
}

// Proves mcp-rag's tool is reachable through the real end-to-end path an MCP client
...
describe('mcp-rag reachable through the mcp-server plugin bridge', () => {
  beforeEach(async () => {
    mockLogger()
    await setupTestDb()
  ...
  test('activation exposes the rag_search tool with an object input schema via listPluginMcpTools', async () => {
    const plugin = discoverMcpRag()
    ...
    expect(pluginRegistry.getEntry('mcp-rag')?.state).toBe('active')

    const tools = await listPluginMcpTools('mcp-rag')

    expect(tools.map((tool) => tool.name).sort()).toEqual([...RAG_TOOL_NAMES].sort())
  ...
  test('returns [] once the plugin is deactivated', async () => {
    ...
    pluginRegistry.evaluateCompatibilityAcrossInstances([])
    await activatePlugins(pluginRegistry.getApprovedCompatiblePlugins())
    expect(await listPluginMcpTools('mcp-rag')).toHaveLength(1)

    await deactivateAllPlugins()

    expect(await listPluginMcpTools('mcp-rag')).toEqual([])
  })
})

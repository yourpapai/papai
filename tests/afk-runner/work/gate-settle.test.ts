...
import { expectedContentFor } from '../../../afk-runner/src/work/gate-settle.js'
...
describe('guarded settle-time expected at an integrity-substituted gate (D3 site 3)', () => {
  it('declares POLICY-INTEGRITY, so a response addressing the substituted blocker parses instead of rejecting as unknown', async () => {
    const h = await makePresentedGate()
    const sidecarDir = path.join(h.runDir, 'sidecars')
    fs.mkdirSync(sidecarDir, { recursive: true })
    fs.writeFileSync(path.join(sidecarDir, 'resolutions-4.json'), '{not json')
    // the harness's round is 4 — the fold's perRound would carry round 4's record
    const perRound = [
      {
        round: 4,
    ...
    const expected = await expectedContentFor(sidecarDir, 4, 'early', perRound)
    expect(expected.blockers.map((blocker) => blocker.id)).toEqual(['POLICY-INTEGRITY'])
    // the operator answers the rendered row: id line + acknowledged + APPROVE
    fs.writeFileSync(
      path.join(h.runDir, 'gate-1.md'),
      '## Gate response\n\nPOLICY-INTEGRITY POLICY-INTEGRITY\n→ acknowledged\nAPPROVE\n',
    )
    const result = settledOf(await h.settleFileWith(expected))
    expect(result.outcome).toBe('approve')
    expect(h.log().some(isAnsweredGateEvent)).toBe(true)
  })

  it('without the perRound record the reader keeps its pre-change reading (resume routing untouched)', async () => {
    const sidecarDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afk-settle-norecord-'))
    fs.mkdirSync(sidecarDir, { recursive: true })
    fs.writeFileSync(path.join(sidecarDir, 'resolutions-2.json'), '{not json')
    const expected = await expectedContentFor(sidecarDir, 2, 'early')

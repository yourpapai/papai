...
test.describe('settings/sections/ToolsSection', () => {
  ...
  test('Empty', async ({ sharedPage }) => {
    await switchStory(sharedPage, 'settings-sections-toolssection--empty')
    await expect(sharedPage).toHaveScreenshot()
  })

  test('Preset applied', async ({ sharedPage }) => {
    await switchStory(sharedPage, 'settings-sections-toolssection--preset-applied')
    await expect(sharedPage).toHaveScreenshot()
  })

...
describe('staged download factory', () => {
  ...
  describe('telegram provider', () => {
    ...
    beforeEach(() => {
      ...
      downloadFn = createStagedDownloader({
        downloadFileFromInstance: (_platformInstanceId, sourceProvider, fileId) => {
          if (sourceProvider === 'telegram') return mockFetcher(fileId)
          return returnNull()
        },
      })
    })
    ...
    test('delegates to telegramFetcher for telegram source', async () => {
      const result = await downloadFn('tg_valid', 'telegram', 'telegram-a')
    ...
    test('returns null for missing telegram file', async () => {
      const result = await downloadFn('tg_missing', 'telegram', 'telegram-a')
  ...
  describe('mattermost provider', () => {
    ...
    beforeEach(() => {
      ...
      downloadFn = createStagedDownloader({
        downloadFileFromInstance: (_platformInstanceId, sourceProvider, fileId) => {
          if (sourceProvider === 'mattermost') return mockFetcher(fileId)
          return returnNull()
        },
      })
    })
    ...
    test('delegates to mattermostFetcher for mattermost source', async () => {
      const result = await downloadFn('mm_valid', 'mattermost', 'mattermost-a')
      expect(result).not.toBeNull()
    ...
    test('returns null for missing mattermost file', async () => {
      const result = await downloadFn('mm_missing', 'mattermost', 'mattermost-a')
      expect(result).toBeNull()
    })
  })
  ...
  describe('discord provider', () => {
    beforeEach(() => {
      downloadFn = createStagedDownloader({ downloadFileFromInstance: () => returnNull() })
    })

    test('returns null for discord (not supported)', async () => {
      const result = await downloadFn('discord_123', 'discord', 'discord-a')
      expect(result).toBeNull()
    })
  })
  ...
  describe('unknown provider', () => {
    ...
    test('returns null for unknown provider', async () => {
      const result = await downloadFn('x_123', 'unknown', 'unknown-a')
      expect(result).toBeNull()
    })
  })
})

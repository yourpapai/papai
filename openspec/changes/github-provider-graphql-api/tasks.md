...
- [x] 2.2 Implement `githubGraphql(config, query, variables, operation = 'read')`, exported `GitHubGraphqlError`, Zod envelope schema (loose `data`, strict `errors[].message`), and the observation wiring from the `src/analytics` primitives per design Decisions 3–8. Verify: `bun test tests/plugins/task-provider-github/graphql-client.test.ts`

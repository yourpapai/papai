...
- [ ] 3.1 Run full suite and read failures from the persisted report: `bun run test` then `bun run test:failures`
- [ ] 3.2 Run all static checks: `bun check:full` (lint + typecheck + knip + format + duplicates)

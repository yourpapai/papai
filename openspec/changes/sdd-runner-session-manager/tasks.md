...
- [ ] 1.1 Write failing scripted-key tests for the form reducer module
...
- [ ] 1.2 Implement the form reducer (pure, generalizing the gate input
  reducer's handling) until the tests pass —
  `bun test tests/sdd-runner/session-create-form.test.ts`

## 2. Screen-switch state machine

- [ ] 2.1 Write failing tests extending the session-screen reducer with a
...
- [ ] 2.2 Implement the screen-switch state and render the form lines (title,
  description, footer hints, validation notice) —
  `bun test tests/sdd-runner/tui-session-screen.test.ts`

## 3. Loop driver

- [ ] 3.1 Write failing tests for the looping picker driver: report shown →
...
- [ ] 3.2 Implement the loop in the picker driver around
...
- [ ] 4.1 Wire the harness: `sessionCreate` starts a run from the form's
...
- [ ] 4.2 Delete the readline creation path and the stdin-fix seam; remove
  their now-dead tests, keeping coverage that still pins live behavior —
  `bun test tests/sdd-runner/ && bun run typecheck`

## 5. Gates and docs

- [ ] 5.1 Run the full gate set — `bun run test`, `bun run typecheck`,
  `bun run lint`, `bun security`
- [ ] 5.2 Update `docs/architecture/sdd-pipeline.md` (session screen loop,

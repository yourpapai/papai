...
- [x] 1.1 Write failing cases in `tests/scripts/mutation/score-merger.test.ts` for
...
- [x] 1.2 Write failing cases for `isMergedScore` (accepts a real score; rejects missing fields,
...
- [x] 2.1 Write failing `tests/scripts/mutation/score-fingerprint.test.ts` covering: identical
...
- [x] 2.2 Implement `scripts/mutation/score-fingerprint.ts` — `SCORE_FINGERPRINT_VERSION`,
...
- [x] 3.1 Write failing `tests/scripts/mutation/score-cache.test.ts`: round-trip through
...
- [x] 3.2 Implement `scripts/mutation/score-cache.ts` (`SCORE_CACHE_FILE`, `SCORE_CACHE_VERSION`,
...
- [x] 4.1 Move `resolveErroredGate` and `reportGates` out of `scripts/mutation/changed-files.ts`
...
- [x] 4.2 Add a case to `tests/scripts/mutation/baseline.test.ts` pinning that `resolveRatchet`
...
- [x] 5.1 Write failing `tests/scripts/mutation/incremental-run.test.ts` for
...
- [x] 5.2 Implement `scripts/mutation/incremental-run.ts` — `IncrementalPlan`, `GateInput`,
...
- [x] 6.1 Write the failing **headline** case in `tests/scripts/mutation/changed-files.test.ts`:
...
- [x] 6.2 Wire `changedFilesRun` (`scripts/mutation/changed-files.ts:144`): add
...
- [x] 6.3 Add `--no-score-cache` to `parseChangedFilesCliArgs` (`:112-142`) and the usage string
...
- [x] 7.1 Add SHA-pinned `actions/cache/restore` before, and `actions/cache/save` (with
...
- [x] 8.1 Add `docs/adr/0424-incremental-mutation-measurement-with-whole-branch-gate.md`,
...
- [x] 8.2 Update `scripts/mutation/README.md` (new "Incremental measurement" section: cache
      ...
      Verify: `bun run format:check`
- [x] 8.3 Manual end-to-end on a scratch branch: run `bun test:mutate:changed
...
- [x] 8.4 Full check-out: `bun test`, `bun run typecheck`, `bun run lint`,
      `bun run workflows:lint`.
      Verify: all green

## Verification record

Task 8.3 was run on a throwaway `scratch/mutation-proof` branch (since deleted) because every
file in this change lives under `scripts/` or `.github/`, which are outside the gateable set —
the change cannot demonstrate itself on its own diff.

| Run | Branch state | Measured | Reused | Wall clock | Verdict |
| --- | --- | --- | --- | --- | --- |
| 1 | commit A adds an untested branch to `src/message-edit/classify.ts` | 1 | 0 | 37s | exit 1 — `classify.ts 0.8696 < 1.0000` |
| 2 | commit B edits an unrelated `src/tools/tool-aliases.ts` | 1 (`tool-aliases.ts`, scored 1.0) | 1 (`classify.ts` @ 0.8696) | 2m51s | exit 1 — **still** `classify.ts 0.8696 < 1.0000` |
| 3 | no change | 0 | 2 | 0.13s | exit 1 — still `classify.ts` |
| 4 | a covering test added to `tests/message-edit/classify.test.ts` (source untouched) | 1 (`classify.ts` re-measured → 0.9130) | 1 | 22s | exit 1 — `0.9130 < 1.0000` |

Run 2 is the headline requirement: the push that changed only `tool-aliases.ts` measured only
`tool-aliases.ts`, yet still failed naming the file that regressed one commit earlier. Run 3
shows the whole-branch verdict costs nothing to re-assert. Run 4 shows the guard invalidating on
a test-only edit — the "faster is not weaker" half.

Suite status at close: `bun run test` → 13910 pass / 1 fail / 3 skip. The single failure is
`tests/sdd-runner/orchestrator.test.ts`, which fails identically on `origin/master` in a clean
worktree (1 pass / 15 fail / 14 errors in isolation) and touches nothing in this change.
`typecheck`, `lint`, `workflows:lint`, `knip` and `format:check` are all clean.

...
- [x] 4.1 Add failing tests for `runAggregatedBuild`: after all batches, one `check:full` over `git add -N .; git diff baselineSha`; file→batch attribution via build output + `git diff --name-only` + claimed files (`spans` ∪ `targetFiles`); passing batches proceed, failing batch members go `needs_human` with stderr and are not merged. Verify: `bun test tests/review-loop/batch-verify.test.ts` (fails)
- [x] 4.2 Implement `runAggregatedBuild` in `review-loop/src/build-checker.ts` and wire the verification phase (`review-loop/src/batch-verify.ts`) between the batch loop and the merge. Update `RoundCollector`/`metrics` to count one `build` per round, with `phaseMs.build` as the single aggregated duration. Verify: `bun test tests/review-loop/batch-verify.test.ts`

## 5. Single aggregated inspector per round

- [x] 5.1 Add failing tests for aggregated inspector: one `runAgent` over full diff + member list, returns per-id `{id, addresses, reasoning}`; `addresses:false` members go `needs_human` and are not merged; `unavailable` treats all `fixed:true` members as `needs_human`. Verify: `bun test tests/review-loop/issue-inspector.test.ts tests/review-loop/batch-verify.test.ts` (fails)
- [x] 5.2 Implement aggregated `runAggregatedInspector` in `review-loop/src/issue-inspector.ts` (new `AggregatedInspectorResultSchema`) and prompt `buildAggregatedInspectPrompt`; wire it after the aggregated build; commit surviving batches (`fix(review-loop): <title> (+N)`) and publish the stacked commits via one `pool.mergeWorkerIntoPrimary` under primary lock, respecting `mergeEachFix`. Verify: `bun test tests/review-loop/batch-verify.test.ts tests/review-loop/loop-controller.test.ts`

## 6. Budget-aware deferral

- [x] 6.1 Add failing tests for deferral: with `runTimeoutMs` set, remaining time gates `low`/`cleanup` batches first; `critical`/`high`/caller-exposed defects are never deferred; deferred entries stay `discovered` with `latestSeenRound` bumped and appear as `deferred` in summary/metrics, re-considered next round. Verify: `bun test tests/review-loop/batch-defer.test.ts tests/review-loop/stop-controller.test.ts` (fails)
- [x] 6.2 Implement `shouldDeferBatch` in `review-loop/src/batch-defer.ts` using remainingMs (median batch duration, no new config; `remainingMs` on `StopController`); surface `deferred` count in `summary.ts` and `metrics.json`. Verify: `bun test tests/review-loop/batch-defer.test.ts tests/review-loop/summary.test.ts`

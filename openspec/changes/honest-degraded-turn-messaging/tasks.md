...
- [ ] 2.2 Implement: add `noopFallback` to the completion dictionary type (`src/i18n/types.ts`) and both locales (wording per design D3), select the fallback by `hadToolActivity` at the two fallback returns in `buildVerifiedCompletion`. Verify: `bun test tests/completion/`

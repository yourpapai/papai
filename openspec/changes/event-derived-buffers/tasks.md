...
- [x] 4.3 Update affected `docs/architecture/` pages (the debug/settings server surfaces in `overview.md`; any dashboard-buffer lifecycle wording in `behaviors.md`) to describe persistent capture with broadcast/read-time visibility. Verify: `bun run knip`
- [x] 4.4 Full gate: complete `bun run test`, `bun run typecheck`, `bun run lint`. Verify: `bun check:full && bun run test`

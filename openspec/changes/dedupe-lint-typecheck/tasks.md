...
- [x] 6.2 End-to-end staged-path check on a clean tree. Verify: `./scripts/check.sh --skip-tests` exits 0.
- [x] 6.3 Full gates. Verify: `bun run test` exits 0, `bun run typecheck` exits 0, `bun run lint` exits 0 (all three run, whatever branch was taken).

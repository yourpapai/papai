...
- [ ] 2.1 Create a scratch `git worktree add` checkout so planted errors never touch the main tree's hook pipeline. Verify: `git worktree list` shows it; main tree `git status` clean.
- [ ] 2.2 Plant the six probes (design.md D2 table) one class at a time in a scratch `src/` file and record which of (a) lint-as-configured, (b) lint with type-check off, (c) `tsgo` reports each. Verify: all six matrix rows filled in design.md `Measured results`; worktree removed afterwards (`git worktree list` shows only the main tree).
- [ ] 2.3 Enumerate `.oxlintignore` vs `tsconfig.json` include/exclude and record any file class one pass checks and the other does not (non-subsumed scope). Verify: finding recorded in design.md `Measured results` (or `no scope differences` noted).

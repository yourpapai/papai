...
The C1–C7 window is a proof-of-concept: quality gates are selectively relaxed for the ported/copied material and re-tightened at the C7 reflection, deliberately earlier than U9. Concretely: jscpd ignores `tests/afk-runner/fixtures/**` and ported test files (near-identical by design — they are the oracle); ported test expectations are never rewritten. Oxlint relaxations, all scoped and C7-re-tightenable: the `max-lines` pair turns off for `afk-runner/**` (safety valve against mid-spike forced splitting — current file estimates fit the defaults; the per-state module layout stays on merit); `typescript/no-unsafe-type-assertion` turns off for `afk-runner/**` because xstate v5's canonical `setup({ types: { context: {} as T } })` idiom cannot be spelled assertion-free at runtime (ambient `declare` values are ReferenceErrors under Bun's eager module evaluation); the `no-unsafe-*` family turns off for `tests/afk-runner/**` because tsgolint (oxlint's type-aware backend) fails to resolve the workspace's xstate-inferred types across module boundaries — tsc is authoritative and green, and the false positives land only in these tests. Gates that are never relaxed: TDD write hooks on new `afk-runner/src/**` files, typecheck, lint otherwise. All relaxations and their re-tightening are recorded in tasks 1.6, 2.0, and C7's scope.

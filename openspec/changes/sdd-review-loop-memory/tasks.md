...
- [x] 2.1 Red-first `concernDigest`: groups resolutions by fingerprint, renders round-tagged one-line-per-concern (`r<last> [id] class resolution — note (seen r<first>..r<last>)`), caps at `LEDGER_DIGEST_MAX` with overflow note; replaces `ledgerLines` in `buildReviewerPrompt` (extracted to `sdd-runner/src/review-prompts.ts`; `concernDigest` itself stays in `concern-model.ts`) — `bun test tests/sdd-runner/review-model.test.ts`
...
- [x] 3.2 Implement the fold + `stampEvent` additive field in `sdd-runner/src/{events,replay}.ts`; maintain `sidecars/concerns.json` at round close in `closeRound` (`sdd-runner/src/review-round.ts`), written after materialization and before the `round_close` event so an interrupted boundary still leaves the history the next round compares against — `bun test tests/sdd-runner/replay.test.ts`
- [x] 3.3 Red-first detection: a fingerprint with ≥2 prior resolved/dismissed entries raised again, or re-raised at a different class, stops the loop and rides its history onto the gate (no new gate mode). Shipped shape, after the merge with `sdd-runner-open-vs-raised`: `detectConcernThrash` ends the loop in `review-loop.ts` with `recurringConcerns` on the result; `routeCapHit` (`post-review-tail.ts`) then denies the verification round that change would otherwise owe a `needs-review` cap-hit, but does **not** itself force an early gate — the open set already decides whether a human is needed, because a nitpick-only cap-hit must flow through under that change's spec. `gate-digest.ts` passes `concernHistory` regardless of gate mode, so whichever gate follows carries the round-by-round history. — `bun test tests/sdd-runner/orchestrator.test.ts tests/sdd-runner/post-review-tail.test.ts`
...
- [x] 4.2 Implement the deterministic scan in `sdd-runner/src/artifact-consistency.ts` (extracted from `materialize.ts` when the merge pushed that file past `max-lines`; its tests stay in `tests/sdd-runner/materialize.test.ts`) with synthesized-finding injection into the next round's findings in `review-agents.ts` — `bun test tests/sdd-runner/materialize.test.ts`
...
| ---------- | ---------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| 2026-08-30 | In-plan, stale anchors | Task 2.1 `buildReviewerPrompt` location                       | `review-model.ts` → `review-prompts.ts` (split at `max-lines` during the merge)                                              |
| 2026-08-30 | In-plan, stale anchors | Task 3.2 `concerns.json` writer                               | `review-loop.ts` → `closeRound` in `review-round.ts`                                                                         |
| 2026-08-30 | In-plan, stale anchors | Task 4.2 consistency scan location                            | `materialize.ts`/`review-loop.ts` → `artifact-consistency.ts`/`review-agents.ts`                                             |
| 2026-08-30 | In-plan, divergent     | Task 3.3 thrash produces an early-gate variant                | Code wins: thrash stops the loop and denies the verification round; the open set decides gating; history rides the next gate |
| 2026-08-30 | Cross-change conflict  | Non-goal "per-round artifact snapshot retention" declined     | Amended in `proposal.md`: superseded by `sdd-runner-open-vs-raised`, which requires the snapshots                            |

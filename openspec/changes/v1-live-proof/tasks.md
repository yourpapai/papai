...
- [ ] 7.2 Convert the four StageId `as`-narrowings (drive/loop.ts ×3, drive/failure-budget.ts ×1) to zod-parse. Verify: `bun test tests/afk-runner/drive/` green and `bun run typecheck`
- [ ] 7.3 Extract the C6 error-taxonomy classes to their own module, resolving the `max-classes-per-file` conflict by extraction. Verify: `bun test tests/afk-runner/` green and `bun run typecheck`
...
- [ ] 8.1 Run the full gate: `bun test`, `bun run typecheck`, `bun run lint`, `bun run duplicates`, and confirm `docs/architecture/` pages affected by this change are current (afk-runner.md; AGENTS.md docs-table row if stale)

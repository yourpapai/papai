...
- [ ] 3.1 Add failing test coverage for alert creation pinning: alert created in a context with instance A configured is pinned to A; context with NULL instance stays NULL (derive config context from the built delivery target). Verify: `bun run test tests/deferred-prompts/tool-handlers.test.ts`

...
- [x] 6.3 Add the memoized `withClaude` wrapper around the run-driving harness members in `sdd-runner/src/index.ts` (guard + `openClaudeContext` on first use) and the best-effort teardown in `runEntry`'s `finally`. Verify: `bun test tests/sdd-runner/index.test.ts && bun run typecheck`
...
- [x] 7.2 Make the chain pass without changing pricing semantics — expected to be test-only; if any wiring gap appears, fix it in `sdd-runner/src/usage-aggregate.ts` without touching the opencode route's numbers. Verify: `bun run sdd-runner:test`
...
- [x] 8.2 Record one credentialed manual run (`bun run sdd-runner:start` against a small task with `"backend": "claude"`) reaching the gate, noting argv, spend and gate digest in the change folder — never in CI. Verify: the run reaches its gate and the notes land in `openspec/changes/sdd-runner-claude-cli-backend/`
- [x] 8.3 Validate the change artifacts. Verify: `bunx openspec validate sdd-runner-claude-cli-backend --strict`
- [x] 8.4 Run the full suite and refresh any other affected `docs/architecture/*.md` page. Verify: `bun test && bun run typecheck && bun run lint`

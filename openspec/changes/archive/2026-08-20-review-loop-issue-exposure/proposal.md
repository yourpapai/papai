...
`#4e816232 [high]` at `context-vault-indexer/daemon.ts` — zero non-test callers — and six of
seventeen fix commits landed in that unreachable package before the run was killed at 297
minutes with issues open. Risk is hazard × exposure; the loop has no exposure term.
...
time this design let a *judgment* control anything.
...
measured survivors. That works because mutation score has an **oracle**. Reachability
has none: caller analysis is blind to this repo's `plugin.json` `main` dispatch and
...
- **Advisory only.** Exposure never blocks dispatch or consumes retry budget. It
...
- **Second-actor cross-check.** The fixer, which has already read the code, reports exposure
  independently — as it does for `severity`. Divergence is recorded per issue.
- Together these measure the loop's own honesty. The divergence record has one named reader:
  the later change that decides whether exposure may gate. This change does not gate.
...
- `mutation-improve`'s select gate — same waste, but a runner-side fix and its own change.

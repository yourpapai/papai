...
- [x] 5.4 Stop/steer acks and announcements: failing tests for localized acks and bot-authored announcement texts, then migrate the user-facing strings in `src/bot.ts`, `src/llm-orchestrator-*.ts`, `src/announcements.ts`. Verify: `bun test tests/announcements/ tests/llm-orchestrator/`

...
- [x] 5.1 Write failing tests in `tests/client/settings/fetcher-schemas-llm-providers.test.ts`: `PublicProviderAccountSchema` parses `modelHints` (default `{}`), `ProviderInput`/patch accepts optional `modelHints`. Verify: `bun test tests/client/settings/fetcher-schemas-llm-providers.test.ts`
- [x] 5.2 Implement client schema updates in `client/settings/fetcher-schemas-llm-providers.ts`. Verify: `bun test tests/client/settings/fetcher-schemas-llm-providers.test.ts`

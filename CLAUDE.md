...
**Cross-thread memory bridge (experimental, default OFF)** — with `cross_thread_memory` ON (same `tool_context_flags` key + super-admin Feature flags section; resolved via `resolveCrossThreadMemoryFlag` in `src/tools/feature-flags.ts`), the bot stops rediscovering recurring facts in every new thread of a group. The mechanism lives in `src/long-term-memory/` and extends the existing long-term store: `memory_records` gains a `provisional` status + `thread_context_id` (migration `056`). (1) **Capture** — an idle-debounce pipeline (`capture.ts` + `capture-debounce.ts`, armed every group-thread turn from `llm-history.ts`; `memory-capture-sweep` scheduler backstop over a `memory_extraction_state` watermark) runs the SMALL_MODEL extractor and writes **provisional**, thread-tagged, embedding-populated records — capturing durable facts even from short threads that never trim. (2) **Recall** — the `recall` tool (registered `normal` mode only, flag-gated) runs a server-side cascade (`recall-cascade.ts`): current-thread provisional → active group memory → sibling-thread provisional (only when 1+2 are weak), hybrid semantic+term-overlap, results tagged with provenance. (3) **Promotion** — when a provisional fact clusters across ≥`MEMORY_PROMOTION_MIN_THREADS` (3) distinct threads and a SMALL_MODEL confirms it is durable/general, it is promoted to `active` group memory (`promotion.ts`; `memory-promotion-sweep` backstop). `memory:promote` events carry counts only. The provisional/promotion store lives in `src/long-term-memory/provisional-store.ts` (shared `recordScopeCondition` in `record-conditions.ts`), re-exported from `store.ts`. The settings-UI Memory section surfaces these provisional records in a separate **Pending (provisional)** subsection (`GET /settings/api/memory` returns `active` + `provisional` via `listMemoryRecords({ statuses })`; `archived`/`stale` stay hidden). Flag OFF ⇒ no provisional capture, `recall` unregistered, `search_memory` unchanged — reference-identical.

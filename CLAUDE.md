...
- `bun test` — all server-side suites; **excludes client and E2E** via `bunfig.toml`.
...
- `src/chat/` — `ChatProvider` interface (`types.ts`), provider registry (`registry.ts`, `createChatProviderFromConfig`; merges builtin + plugin-contributed factories), `ChatRouter` fan-out (`router.ts`), command-menu registration (`startup.ts`), and the now-inert `interaction-router.ts`. Chat providers (Telegram, Mattermost, Discord, Kontur Talk) are now plugin-contributed via `plugins/chat-provider-{id}/`.
...
- **Permissions (MVP)** — `storage`, `scheduler`, `commands`, `tasks.read`, `tasks.write`, `provider.task`, `provider.chat`, `identity`, `http`. Raw chat send, raw provider/DB access, and unallowlisted network access are not exposed.
- **Chat provider plugins** — declare `contributes.chatProviderTypes` (max 1), `provider.chat` permission, and `chatProviderFactory` (named export for early-pass factory registration). Manifest fields: `chatProviderCapabilities`, `chatProviderTraits`, `chatProviderThreadCapabilities`, `providerConfigSchema`, `providerAllowedHosts`. Four built-in providers (Telegram, Mattermost, Discord, Kontur Talk) are migrated to `plugins/chat-provider-{id}/`. Auto-approved on first startup via `seedBuiltinChatProviderPlugins()`. Chat provider factories are registered before `ChatRouter` construction so plugin-contributed providers are available at startup.

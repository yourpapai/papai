...
| afk-runner            | [`docs/architecture/afk-runner.md`](docs/architecture/afk-runner.md)                         | graph kernel over xstate pure transition, fold-is-truth engine, grow-not-restore plan (C1–C7 delivered incl. the live proof), living U-ledger, closed relaxation window (re-tighten performed at R5), runner commands (the sole runner since R5 deleted the sdd-runner fallback; rollback is `git revert` of the deletion commit) |

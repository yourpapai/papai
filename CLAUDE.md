...
| SDD pipeline            | [`docs/architecture/sdd-pipeline.md`](docs/architecture/sdd-pipeline.md)                                                                                             | stages, event model, depth profiles, gate protocol (process sections canonical; the `sdd-runner/` workspace was deleted at R5 and the `afk-runner/` tree was later extracted too — the pipeline lives in the external repo `yourpapai/afk-runner`) |
| afk-runner (moved)      | [`docs/architecture/afk-runner.md`](docs/architecture/afk-runner.md), [`docs/architecture/afk-runner-mcp-research.md`](docs/architecture/afk-runner-mcp-research.md) | tombstone row: the runner and both docs moved to `yourpapai/afk-runner`; the two papai paths are one-line pointer files kept only so retained openspec links resolve                                                                               |
...
| Autonomous SDD pipeline (`/sdd:auto`)           | moved to `yourpapai/afk-runner` — the pipeline and its command live there; papai has no runner (the `/sdd:auto` command files are tombstone stubs)              |

...
| SDD pipeline          | [`docs/architecture/sdd-pipeline.md`](docs/architecture/sdd-pipeline.md)                     | stages, event model, depth profiles, gate protocol (process sections canonical; the sdd-runner workspace is frozen off-primary)                                                                                                                                                                   |
| afk-runner            | [`docs/architecture/afk-runner.md`](docs/architecture/afk-runner.md)                         | graph kernel over xstate pure transition, fold-is-truth engine, grow-not-restore plan (C1–C7 delivered incl. the live proof), living U-ledger, closed relaxation window, runner commands (the R4 cut-over routes every operator entry point here; sdd-runner stays as a one-line-revert fallback) |

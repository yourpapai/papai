...
ACP coding sessions (`plugins/acp/`): a thin first-party plugin exposing LLM tools (start/list/status/finish/cancel a coding session, answer a permission, review a PR, list projects/agents), an `/acp` command, and a prompt fragment. It is a stateless HTTP client of the external **magi** control service (admin-scoped `magi_base_url` + sensitive `magi_token` plugin config; outbound via `providerRuntime.httpFetch`, with the admin base URL allowlisted through `providerAllowedHostsFromConfig`). It injects the chat's `storageContextId` as the magi session `contextId` (so magi's milestone notifier posts back to `/api/notify`) and uses `plugin_kv` to scope `list_sessions` to the originating chat. Because plugin source cannot static-import bare modules (discovery rejects them), the tools use raw JSON-Schema `inputSchema` with manual guards instead of Zod. The sandboxed agent's LLM key is supplied **per-session** from the `coding_session_credentials` vault (`namespace: 'agent-provider'`) via the `coding.secrets`-gated `codingSecrets` capability (no host/global agent key is used or expected); `start_session` and `review_pr` refuse with `error: 'not_configured'` when the vault is empty for the acting context.

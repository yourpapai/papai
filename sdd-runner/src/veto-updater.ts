...
async function spawnVetoUpdater(input: AttemptInputs, lastError: string | null): Promise<readonly string[]> {
  const basePrompt = buildVetoUpdaterPrompt({
    ...
    reportPath: agentWritePath(input.ctx.cwd, 'veto-updater.json'),
  })
  const prompt = lastError === null ? basePrompt : `${basePrompt}\n\nPrevious attempt failed validation:\n${lastError}`
  const result = await runStageAgent(input.deps.agent, {
    role: 'resolver',
    changeName: input.changeName,
    cwd: input.ctx.cwd,
    prompt,
...
async function attemptVetoUpdater(
  ...
  const filesUpdated = await spawnVetoUpdater(input, lastError)
  const validation = await input.deps.driver.validateStrict(input.changeName)
  if (validation.ok) return { filesUpdated }
  ...
  return attemptVetoUpdater(input, attempt + 1, validation.output)
}

export async function runVetoUpdater(
  ...
  const [artifacts, assumptions, findings] = await Promise.all([
    readArtifactContent(ctx.changeDir),
    readAssumptionLookup(ctx.sidecarDir, state.round),

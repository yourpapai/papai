  ...
  return (cwd, args) => {
    ...
    if (!READABLE_GIT_SUBCOMMANDS.has(subcommand)) {
      return Promise.reject(
        new Error(`read-only git seam rejected '${subcommand || '<none>'}' (allowed: log, ls-tree)`),
      )
    }
...
export interface RunBundle {
  ...
  readonly expiryClaimVersions: readonly number[]
...
/**
 * Load one run dir degraded-gracefully: every artifact (state, sidecars, gate
...
export async function loadRunBundle(fs: AnalyzeFs, workDir: string, runId: string): Promise<RunBundle> {
  ...
  return {
    ...
    gateFiles: gateFiles.gateFiles,
    expiryClaimVersions: gateFiles.expiryClaimVersions,
  }
}
...
const EXPIRY_CLAIM_RE = /^gate-(\d+)\.expiry-claim$/u
...
interface GateFileLoad {
  readonly gateFiles: readonly GateFileRecord[]
  readonly expiryClaimVersions: readonly number[]
}

async function readGateFiles(fs: AnalyzeFs, runDir: string): Promise<GateFileLoad> {
  const entries = await fs.readdir(runDir).catch(() => [] as string[])
  const gateNames = [...entries].sort().filter((name) => GATE_FILE_RE.test(name) || EXPIRY_CLAIM_RE.test(name))
  const contents = await Promise.all(
  ...
  const expiryClaimVersions: number[] = []
  for (const { name, md } of contents) {
    if (md === null) continue
    const gateMatch = GATE_FILE_RE.exec(name)
    if (gateMatch !== null) {
      gateFiles.push({ version: Number(gateMatch[1]), md })
      continue
    }
    const claimMatch = EXPIRY_CLAIM_RE.exec(name)
    if (claimMatch !== null) expiryClaimVersions.push(Number(claimMatch[1]))
  }
  return { gateFiles, expiryClaimVersions: expiryClaimVersions.sort((a, b) => a - b) }
}

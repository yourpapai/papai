...
export type PluginContextLike = {
  ...
  providerRuntime: { httpFetch: HttpFetch | undefined }
...
function isGetter(value: unknown): value is (key: string) => string | undefined {
  return typeof value === 'function'
}
...
export function requirePluginContext(ctx: unknown): PluginContextLike {
  ...
  return {
    ...
    ...(isRecord(providerRuntime)
      ...
      : {}),
    ...(isRecord(adminConfig) && isGetter(adminConfig['get'])
      ? {
          adminConfig: {
            get: adminConfig['get'],
          },

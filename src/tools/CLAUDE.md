...
- chat-history search: `search_chat_history` FTS5 keyword-searches observed messages in `message_metadata` (group-wide in groups, DM-scoped in DMs); available in both DM and group contexts whenever `storageContextId` + `chatUserId` are set

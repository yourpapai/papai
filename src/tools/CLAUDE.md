...
- chat-history search: `search_chat_history` FTS5 keyword-searches observed messages in `message_metadata` (group-wide in groups, DM-scoped in DMs); available in both DM and group contexts whenever `storageContextId` + `chatUserId` are set. `get_message` fetches a single message by id and `get_message_context` returns a temporal/thread/reply_chain window around a message; both share the same scope enforcement and return `not_found` for out-of-scope ids (no existence leak)

...
- diagnostics readers: `read_recent_logs` / `read_llm_traces` / `read_recent_turns` / `read_recent_tool_failures` (one file each under `src/tools/diagnostics-*.ts`) assemble beside `run_diagnostics` under the same fail-closed gate, binding `options.chatUserId` as the visibility principal at assembly time (safe because they only assemble in the admin's own DM). They read the always-on in-process diagnosis buffers read-only — never mutate them — with per-probe try/catch degrading to `probe_error` and metadata-only logging. Egress reuses the exact dashboard code paths: logs mirror `handleLogs` (own entries — explicit `chatUserId` match or turn attribution via `ownTurnIdsForAdmin`, resolved once per call — pass verbatim; foreign/unattributed entries are shaped to structural + numeric/boolean fields, and the caller's `level`/`scope`/`turn_id`/`msg` filters run against **post-shaping** content so a text filter can never probe stripped content; limit default 50 / cap 200; `distinct_scopes` returns scope/count pairs); traces go through `shapeLlmTrace` per trace (own verbatim; foreign lose `generatedText`/`stepsDetail`/identity/tool `args`+`result`, keep model ids/durations/token+step counters/tool names/`error`; `errors_only`/`model` filters; default 25 / cap 100 from the tail); turns and tool failures filter with `isVisibleToAdmin(scope, clientVisibility(chatUserId))` like the dashboard init frame — turn payloads are anonymous operational data only (timings/status/tool names/durations/failureReason/error), a `turn_id` fetch returns `not_found` indistinguishably for foreign/invisible/unknown ids (no existence leak, includes in-flight running turns via `findTurnById`), and failure egress is a fixed whitelist (timestamp, scope, toolName, durationMs, ok, failureReason, turnId); turn limits default 25 / cap 512 and failure limits 25 / cap 1024 (= the buffer capacities). Every result carries the buffer's derived stats (count/capacity/oldest/newest) and log results add `history_starts_at_process_start: true` — the buffers are volatile, so after a restart readers return empty results with zero-count stats rather than errors. All four are registered `read('diagnostics')` in `tool-metadata.ts`: implicit `allow` default, `deny` removes them from the set, `ask` wraps each call in the confirmation flow, and the sticky presets never gate read-risk tools.
- proof checks (DISPOSABLE — removed together with the `src/deferred-prompts/proof-checks*` module and the `poller.ts` record line): `run_proof_check` (`src/tools/proof-check-run.ts`) and `read_proof_results` (`src/tools/proof-checks-read.ts`) assemble beside the readers under the same fail-closed admin/DM/normal gate, binding `options.storageContextId`/`options.chatUserId` at assembly time (an empty binding degrades to the runner's structured error). `run_proof_check` drives the five e2e probes from `src/deferred-prompts/proof-checks.ts` — sync checks return the finished record inline, async checks return `{status:'started', run_id}` and finish in the background (second concurrent run: structured `busy`), and `cleanup: true` sweeps leftover `[[proof-check:<runId>]]`-marked prompts owned by the admin's storage context; it is registered `write('diagnostics', 'create')` — a write-risk tool (it creates real prompts and spends LLM tokens), so `deny` removes it, `ask` wraps it per-call, and the sticky `read-only` preset asks on it. `read_proof_results` lists the capped JSONL proof store most-recent-first with `run_id`/`limit` filters and is `read('diagnostics')` like the readers. The one prod-path touch is the D9 poller record: after `sendProactiveMessage` resolves `true`, `recordProofGroupDelivery` in `src/deferred-prompts/poller.ts` appends `{runId, responseText, delivered: true, at}` to the proof store when every prompt in the delivery group carries the marker (mixed proof+real groups skip — a merged response cannot be attributed); a pure record that never alters the message, the target, or control flow.

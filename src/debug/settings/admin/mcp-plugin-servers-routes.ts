...
const log = logger.child({ scope: 'debug-server:settings-admin-mcp-plugin-servers' })

const PostBodySchema = z.object({ kind: z.literal('plugin-servers'), configs: mcpPluginServerConfigsSchema })
...
function view(pi: string): Response {
  return settingsJson(200, { available: availablePluginServers(), configs: resolveMcpPluginServerConfigs(pi) })
}
...
async function handlePost(req: Request, authed: AuthenticatedSettingsRequest): Promise<Response> {
  ...
  setMcpPluginServerConfigs(pi, body.data.configs)
  log.info({ platformInstanceId: pi }, 'MCP plugin server configs updated')

  return view(pi)
}

export function handleAdminMcpPluginServersRoutes(req: Request, _url: URL, pathname: string): Promise<Response> {
  const auth = authenticate(req)
  if (!auth.ok) return Promise.resolve(auth.response)
  if (pathname === '/settings/api/admin/mcp-plugin-servers') {

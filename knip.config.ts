...
import { KNIP_ENTRY_POINTS } from './knip-entries.js'
...
export default {
  ...
  // Entry points not auto-detected from package.json scripts live in
  // knip-entries.ts (extracted for the max-lines cap recorded in the header).
  entry: KNIP_ENTRY_POINTS,
  ...
  ignoreIssues: {
    ...
    // The settings UI preview seam of chat-model-metadata-models-dev: the hint
    // component and its fetcher are wired into RoleBindingBlock/ProviderForm by
    // the next task of the same change; until then tests read their exports.
    'client/settings/components/ModelMetadataHint.svelte': ['exports', 'types'],
    'client/settings/llm-model-metadata-fetchers.ts': ['exports', 'types'],
